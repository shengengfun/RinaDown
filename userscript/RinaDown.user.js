// ==UserScript==
// @name              RinaDown 下载接管 / RinaDown Download Capture
// @name:zh-CN        RinaDown 下载接管
// @namespace         https://github.com/zerx-lab/RinaDown
// @version           1.0.0
// @description       拦截浏览器下载与流媒体资源，一键发送到 RinaDown 桌面下载器（通过本地 RPC）。Intercept downloads & media, send them to the RinaDown desktop app via local RPC.
// @description:zh-CN 拦截浏览器下载与流媒体资源（HLS/DASH/视频/音频/压缩包/安装包等），一键发送到 RinaDown 桌面下载器。
// @author            zerx-lab
// @license           MIT
// @match             *://*/*
...
// @noframes
// @homepageURL       https://github.com/zerx-lab/RinaDown
// @supportURL        https://github.com/zerx-lab/RinaDown/issues
// ==/UserScript==
 ...
 *   3. 通过 GM_xmlhttpRequest POST 到 RinaDown 的本地 HTTP 接管服务
 *      （默认 http://127.0.0.1:17800/download），由桌面端弹出确认框后下载。
 *
 * 与浏览器扩展的能力差异（务必知悉）
 * ----------------------------------
 *   - 无法拦截「浏览器内核直接发起」的下载（非页面 JS 触发的、点击后直接由
 *     Content-Disposition 触发的下载）——这类请在 RinaDown 浏览器扩展中完成；
 ...
 *   - 每个请求携带 X-RinaDown-Client 头（RinaDown 据此拦截恶意网页的跨域伪造请求）；
 *   - 可在菜单里设置 Token（与 RinaDown 设置页一致）做额外鉴权；
 *   - 最终所有下载都会在 RinaDown 弹出确认框，不会静默下载。
 *
 * 配置（点击油猴菜单 → RinaDown ...）
 * -----------------------------------
...
(function () {
  ...
  if (W.__rinadown_userscript__) return;
  W.__rinadown_userscript__ = true;
  ...
  // 把单个下载请求发给 RinaDown。返回 true 表示已被接受。
  async function sendDownload(payload) {
    try {
      ...
      if (!ok) console.warn('[RinaDown] send failed:', r.status, r.responseText);
      return ok;
    } catch (e) {
      console.warn('[RinaDown] send error:', e);
      return false;
    }
  }

  // 批量下载（单次请求，url 由 RinaDown 端按换行拆分，用户只需确认一次）。
  async function sendBatch(urls, shared) {
    ...
    try {
      ...
      console.warn('[RinaDown] batch error:', e);
      return false;
    }
  }
  ...
  async function takeover(url, opts) {
    ...
    if (ok) {
      toast(`已发送到 RinaDown：${opts.filename || filenameOf(url) || url}`);
    } else {
      // 失败：确认 App 是否在线，决定是否回退浏览器下载。
      const alive = await ping();
      if (alive) {
        toast('RinaDown 收到请求（响应异常），请查看桌面端', true);
      } else {
        toast('RinaDown 未运行，已交回浏览器下载', true);
  ...
  function browserFallback(url, filename) {
    ...
    try {
      ...
      a.setAttribute('data-rinadown-skip', '1');
  ...
  document.addEventListener('click', function (ev) {
    ...
    const a = ev.target && ev.target.closest
      ...
      : null;
    if (!a) return;
    if (a.hasAttribute('data-rinadown-skip')) return;
  ...
  try {
    ...
    W.HTMLAnchorElement.prototype.click = function () {
      try {
        if (CFG.enabled && !this.hasAttribute('data-rinadown-skip')) {
  ...
  function buildUI() {
    ...
    host.id = 'rinadown-userscript-root';
  ...
  async function sendAllSniffed() {
    ...
    toast(ok ? `已发送 ${urls.length} 个资源到 RinaDown` : '发送失败，请确认 RinaDown 在运行', !ok);
  }
  ...
  async function downloadAllLinks() {
    ...
    const ok = await sendBatch(urls);
    toast(ok ? `已发送本页 ${urls.length} 个链接到 RinaDown` : '发送失败，请确认 RinaDown 在运行', !ok);
  }
  ...
  function toast(msg, warn) {
    try {
      ...
      try { GM_notification({ text: msg, title: 'RinaDown', timeout: 3000 }); } catch (__) { /* */ }
    }
  }
  ...
  function registerMenu() {
    ...
    add(`🔌 设置端口（当前 ${CFG.port}）`, () => {
      const p = prompt('RinaDown RPC 端口（与设置页一致，默认 17800）：', String(CFG.port));
      if (p !== null) {
    ...
    add('🔑 设置 Token（可选）', () => {
      const t = prompt('RinaDown RPC 授权密钥（在 RinaDown 设置页生成，可留空）：', CFG.token);
      if (t !== null) { CFG.token = t.trim(); toast('Token 已保存'); }
    });
    add('🩺 测试连接', async () => {
      toast('正在测试…');
      const ok = await ping();
      toast(ok ? `已连接 RinaDown（${base()}）` : `无法连接 ${base()}，请确认 RinaDown 已启动且 RPC 服务已开启`, !ok);
    });
  }
  ...
  console.log('[RinaDown] userscript loaded; target =', base());
})();
