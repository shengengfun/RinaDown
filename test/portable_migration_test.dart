...
void main() {
  ...
  setUp(() {
    root = Directory.systemTemp.createTempSync('rinadown_portable_migrate_');
    exeDir = root.path;
  ...
  test('首迁：DB 三件套 + 清单条目全部移入，非数据文件原地保留', () {
    write('rina_down.db', 'db');
    write('rina_down.db-wal', 'wal');
    write('rina_down.db-shm', 'shm');
    write('settings.json', '{}');
    write('icons${sep}custom_icon.ico', 'ico');
    write('logs${sep}rinadown_2026-01-01.log', 'log');
    write('rina_down.exe', 'bin');

    migratePortableData(exeDir, newDir);

    expect(read('$newDir${sep}rina_down.db'), 'db');
    expect(read('$newDir${sep}rina_down.db-wal'), 'wal');
    expect(read('$newDir${sep}rina_down.db-shm'), 'shm');
    expect(File('$newDir${sep}icons${sep}custom_icon.ico').existsSync(), isTrue);
    expect(
      File('$newDir${sep}logs${sep}rinadown_2026-01-01.log').existsSync(),
      isTrue,
    );
    expect(File('$exeDir${sep}rina_down.db').existsSync(), isFalse);
    expect(Directory('$exeDir${sep}icons').existsSync(), isFalse);
    // exe 根层的非数据文件不在清单内，必须原地保留。
    expect(File('$exeDir${sep}rina_down.exe').existsSync(), isTrue);
  });
  ...
  test('孤儿 WAL 绝不单独搬运', () {
    write('rina_down.db-wal', 'wal');
    migratePortableData(exeDir, newDir);
    expect(File('$exeDir${sep}rina_down.db-wal').existsSync(), isTrue);
    expect(File('$newDir${sep}rina_down.db-wal').existsSync(), isFalse);
  });

  test('新主库已存在：旧三件套整组原地保留，不混搬 WAL', () {
    write('rina_down.db', 'old-db');
    write('rina_down.db-wal', 'old-wal');
    write('portable_data${sep}rina_down.db', 'new-db');
    migratePortableData(exeDir, newDir);
    expect(read('$newDir${sep}rina_down.db'), 'new-db');
    expect(File('$exeDir${sep}rina_down.db').existsSync(), isTrue);
    expect(File('$exeDir${sep}rina_down.db-wal').existsSync(), isTrue);
    expect(File('$newDir${sep}rina_down.db-wal').existsSync(), isFalse);
  });
}
