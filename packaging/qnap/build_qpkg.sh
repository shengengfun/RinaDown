...
#       imagemagick（convert）用于从 assets/logo/rinadown_logo.png 生成 QDK 要求的 gif 图标。
set -eu
...
chmod 755 "$pkg/shared/rinadown.sh"
...
mkdir -p "$pkg/$ARCH"
cp "$BIN" "$pkg/$ARCH/rinadown-server"
chmod 755 "$pkg/$ARCH/rinadown-server"

# ── 图标（QDK 要求 gif：64x64 / 80x80 / 64x64 灰度） ──
mkdir -p "$pkg/icons"
convert "$LOGO" -resize 64x64 "$pkg/icons/RinaDown.gif"
convert "$LOGO" -resize 80x80 "$pkg/icons/RinaDown_80.gif"
convert "$LOGO" -resize 64x64 -colorspace Gray "$pkg/icons/RinaDown_gray.gif"
