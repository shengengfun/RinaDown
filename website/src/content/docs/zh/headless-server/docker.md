---
title: Docker 与 NAS
description: 用预编译 Docker 镜像运行 headless RinaDown 服务器，支持 Docker Compose、CasaOS/ZimaOS、Unraid 与群晖 DSM 原生套件。
...
镜像：`ghcr.io/zerx-lab/rinadown-server`（标签：具体版本如 `0.1.54`，或 `latest`）。
...
在 docker-compose 或其它编排场景中，可用 `RINADOWN_TOKEN` 预置密钥并跳过向导。仅在实例尚未设置过密钥时生效：
...
services:
  rinadown-server:
    ...
    # environment:
    #   RINADOWN_TOKEN: your-secure-key-here   # 可选：预置访问密钥，跳过初始化向导
    #   RINADOWN_LANG: zh
...
[服务器部署](/docs/zh/headless-server/setup/)中的全部环境变量在此同样适用——最常用的是 `RINADOWN_LANG`（Web 界面默认语言，`en`/`zh`）和 `RINADOWN_DATABASE_URL`（指向外部 PostgreSQL 而非内置 SQLite）。

## CasaOS / ZimaOS

RinaDown 已发布为第三方 CasaOS / ZimaOS 应用商店，可一键安装。
...
然后从商店安装 **RinaDown**。商店源：[zerx-lab/casaos-appstore](https://github.com/zerx-lab/casaos-appstore)。
...
| 套件 | DSM 版本 | CPU |
|---|---|---|
| `RinaDown-Server-<ver>-synology-dsm7-x64.spk` | DSM 7.0 及以上 | Intel / AMD（x86_64） |
| `RinaDown-Server-<ver>-synology-dsm7-arm64.spk` | DSM 7.0 及以上 | ARM64（rtd1296、rtd1619b、armada37xx 等） |
...
- 数据库、日志与访问密钥位于 `/var/packages/RinaDown/var`；下载默认也落在该目录。
- DSM 7 上要下载到共享文件夹，需先给套件用户授权：**控制面板 → 共享文件夹 → 编辑 → 权限**，把用户下拉切到**系统内部用户**，给 **RinaDown** 读写权限。DSM 6 以 root 运行，无需授权。
