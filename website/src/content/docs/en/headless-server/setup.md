---
title: Server Setup
description: Build and run the headless RinaDown server, configure it with environment variables, and expose it safely.
section: headless-server
order: 1
---

`rinadown_server` is a headless build of the RinaDown download engine: no Flutter UI, no Rinf/FFI layer. It exposes the same Rust engine (HTTP/HTTPS, FTP, BitTorrent, HLS, DASH) over HTTP, WebSocket, and a Web UI baked into the executable, so you can run it on a NAS, a home server, or a VPS and manage downloads remotely from a browser. Releases ship a **single self-contained binary** — no companion `web/` folder to copy around.
...
The server lives in the `native/server` crate (package name `rinadown_server`, binary name `rinadown-server`). From the repository root:

```bash
# Development run (debug build, default bind 0.0.0.0:17800)
cargo run -p rinadown_server

# Production build
cargo build --release -p rinadown_server
# binary at target/release/rinadown-server (rinadown-server.exe on Windows)
```
...
cd ..
cargo build --release -p rinadown_server   # embeds web/dist into the binary
```
...
- `RINADOWN_EMBED_WEBROOT` — embed a different directory instead of `web/dist` (used by CI, which builds the SPA in a separate job).
...
<!-- TODO(screenshot): browser showing the first-run “Initialize RinaDown Server” wizard -->
...
|---|---|---|
| `RINADOWN_BIND` | `0.0.0.0:17800` | TCP address the HTTP/WebSocket server listens on. |
| `RINADOWN_DATA_DIR` | Platform auto-detected (see below) | Directory holding the database file and logs. |
| `RINADOWN_DATABASE_URL` | unset — uses a SQLite file inside the data dir | Explicit connection string: `sqlite:/path/to/file.db` or `postgres://user:pass@host/db`. |
| `RINADOWN_WEBROOT` | unset — serves the embedded Web UI | Optional override: serve the SPA from this directory instead of the embedded copy (custom front end, or a hot-swapped `bun run build` output). There is **no** implicit `./web` lookup next to the executable. |
| `RINADOWN_TOKEN` | unset — first-run Web setup wizard | Optional pre-set management access key. Applied only when the database has no key yet (value is trimmed; must satisfy the key rules below, otherwise ignored with a warning). Use for unattended docker-compose / k8s / CI deploys that skip the wizard. |
| `RINADOWN_DEMO` | unset (off) | Truthy value (`1`/`true`/`yes`/`on`) turns on demo mode: only a built-in, generated 64 MiB file can be downloaded. Useful for public demos. |
| `RINADOWN_DEMO_URL` | unset (off) | Overrides demo mode's allowed URL with a specific one instead of the built-in generated file. |
| `RINADOWN_LANG` | unset (falls back to browser language) | Default Web UI language (`en`/`zh`; regional variants like `zh-CN` accepted). Pure fallback: once any user saves a language in Settings, the saved value becomes the server-side default (applies live, survives restarts); users who explicitly picked a language in their browser always keep their own choice. |

When `RINADOWN_DATA_DIR` is not set, the data directory is auto-detected the same way the desktop app does:
...
For a headless deployment you almost always want to set `RINADOWN_DATA_DIR` explicitly to a stable, backed-up path instead of relying on auto-detection.

```bash
RINADOWN_BIND=0.0.0.0:8080 \
RINADOWN_DATA_DIR=/srv/rinadown/data \
./rinadown-server
```
...
```
==============================================================
  RinaDown Server: first run — no access key is set yet.
  Open the Web UI and create one:
    http://<server-ip>:17800/
  Requirements: 8+ characters, letters and digits.
  Unattended deploys can preset it via RINADOWN_TOKEN.
  ---
...
Open that URL. The login page becomes an **Initialize RinaDown Server** wizard (not a normal sign-in form): enter an access key, confirm it, optionally click the button to random-generate one (`fxd_` + 24 characters), optionally check “Remember this device”, then save. You are signed into the main UI immediately — no server restart.
...
To skip the wizard (docker-compose, Kubernetes, CI), preset the key with `RINADOWN_TOKEN`. It is adopted only when the database still has no key:

```bash
RINADOWN_TOKEN='your-strong-key-here' ./rinadown-server
```

### Security note

The setup window is first-come, first-served: whoever reaches the wizard first sets the key. Finish initialization (or preset `RINADOWN_TOKEN`) before exposing the server on an untrusted network.
...
```bash
RINADOWN_DATABASE_URL=postgres://rinadown:password@localhost/rinadown \
cargo run -p rinadown_server
```

The connection string's scheme (`sqlite:` vs `postgres:`) selects the backend; both share the same schema and migrations. Credentials in `RINADOWN_DATABASE_URL` are masked in the server's own log output, but treat the environment variable itself like any other secret (avoid putting it in shell history or committing it to a process manager's config in plaintext where avoidable).

## Exposing it safely (reverse proxy & TLS)

`RINADOWN_BIND` defaults to `0.0.0.0:17800` — reachable on every network interface, unlike the desktop app's local API which is hardcoded to loopback only. That is intentional for headless use, but it means **you** are responsible for the network boundary:
...
- Prefer binding to a private interface (`RINADOWN_BIND=127.0.0.1:17800` and letting the reverse proxy sit in front, or a VPN/Tailscale address) over exposing the port directly to the public internet, even with TLS.
...
```ini
[Unit]
Description=RinaDown headless download server
...
User=rinadown
Group=rinadown
WorkingDirectory=/opt/rinadown
Environment=RINADOWN_BIND=0.0.0.0:17800
Environment=RINADOWN_DATA_DIR=/var/lib/rinadown
ExecStart=/opt/rinadown/rinadown-server
Restart=on-failure
RestartSec=5
...
Drop the `rinadown-server` release binary into `/opt/rinadown` (it carries the Web UI inside it — nothing else to install), create the `rinadown` system user and `/var/lib/rinadown`, then:

```bash
sudo systemctl daemon-reload
sudo systemctl enable --now rinadown-server
sudo journalctl -u rinadown-server -f   # watch for the first-run setup guidance banner
```

Then open `http://<host>:17800/` in a browser and complete the Initialize RinaDown Server wizard (or preset `RINADOWN_TOKEN` in the unit file for unattended setup).
