...
- UI 目标消歧：未来 PC 客户端将从 Flutter 迁移到 GPUI。用户明确说“GPUI 项目”或“GPUI UI”才修改 `crates/app`（包 `rinadown_ui_app`）及 `crates/{i18n,theme,components,shell}`；未明确指定 GPUI 的“RinaDown UI/PC 客户端/UI 层”一律仍指当前默认 Flutter `lib/`。
