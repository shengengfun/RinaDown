...
- `proxy_config.rs`：无/系统（Windows 注册表静态项 + **PAC/WPAD**）/手动/**自动**（`ProxyMode::Auto`）；HTTP/HTTPS/SOCKS4/5；`test_proxy_connection` 测延迟。`ProxyMode::System` **是目标相关的**——`detect_system_proxy_for(target)` / `ProxyConfig::resolve_for(Some(url))`，PAC 与静态项并存时 PAC 优先（脚本判定直连即直连；仅求值失败回退静态），静态项是 `ProxyEnable=1` + `ProxyServer`（多协议格式，`https=` 描述目的地 → 传输仍是 HTTP）。
- `system_proxy_pac.rs`（仅 Windows）：PAC / WPAD 求值。用 Windows 自己的 WinHTTP（`WinHttpGetProxyForUrl`，`WINHTTP_AUTOPROXY_CONFIG_URL` / `AUTO_DETECT`）下载并执行 PAC——与浏览器同款实现，不必给引擎背 JS 引擎。会话进程级复用（句柄存 `usize`，不引入 `unsafe impl Send/Sync`，且用 `WINHTTP_ACCESS_TYPE_NO_PROXY` 直连取 PAC：否则为解析 PAC 又要先求值 PAC），超时收紧到 3-5s，结论按「来源签名 + **目标 URL**」缓存 120s（失败 60s，键含 URL 因为 PAC 可按路径判定）；`clear_system_proxy_cache()` 同时关掉会话以作废 WinHTTP 内的 PAC 脚本缓存。返回的代理列表按注表同构格式解析（`socks=` 前缀 → SOCKS5），空/`DIRECT` = 直连。
- **WinHTTP 的两个实测硬限制 + 脚本兑底**：(1) 不认 `file://` / 裸本地路径（`ERROR_WINHTTP_UNRECOGNIZED_SCHEME`，Shadowsocks-Windows 式 PAC 就是这个形态）；(2) 要求 PAC 响应带 `Content-Type: application/x-ns-proxy-autoconfig`，否则 `text/plain` / `application/octet-stream`（朴素静态服务器）下报 `ERROR_WINHTTP_BAD_AUTO_PROXY_SCRIPT`。两者都表现为「VPN 软件开了 PAC 模式但 APP 不识别」。故 WinHTTP 失败后走 `fallback_from_script`：自己取回脚本（本地直读 / `http://` 用 std TcpStream 极简 GET，`https://` 放弃；不引 reqwest blocking——同步上下文里建 runtime 会 panic），再按 `extract_single_loopback_proxy` 取端点。判据刻意收紧：**整份脚本只出现一个代理端点、且必须是回环地址**（Clash Verge / v2rayN / Shadowsocks 的默认 PAC 都是「全交给本机客户端」这个形状）；多个端点或远端代理一律不猜。
- `auto_proxy.rs`：`ProxyMode::Auto` 决策机器——任务直连无阻塞启动（保留 CDN 聚合资格），越过 6s 爬升期且剩余 ≥4MiB 时，对全部可用候选（手动字段 + 系统代理，**系统代理按任务 URL 求值后才会进候选表**；相同端点去重）并行各采 256KiB；按相同的单连接量纲比较（不让多分段总速掩盖慢连接），取最快且吞吐 ≥2× 的代理，经 `NodePool::switch_to_client` 在分段边界热切换。host 级决策缓存两层：内存租约 `DecisionCache` 记录胜出来源（重启清零）+ `route_health.rs` 持久化先验（config `auto_route_health`，网络指纹 epoch——换网整表丢弃、**离线=unknown 不清表**）；持久层未记录代理来源，故手动与系统候选并存时只缩短复评等待、不盲采纳旧 Proxy 先验。Cooldown 指数退避 / NoSwitch 完整性门禁；局部续传永不直接采纳 host 代理租约，必须重新采样 validator。failover **独立于通用重试配额**：手动代理、系统代理、本地直连在一个自动恢复周期内各尝试至多一次，当前链路传输失败就切到尚未尝试的候选；三路均失败后只服从通用重试，杜绝 ping-pong。任务级最终链路落 `tasks.auto_route` + `EngineEvent::TaskRouteChanged`（含 `direct:failover` / `proxy:failover:{manual,system}`），双端详情面板可追溯。代理设置变更同点清 `DecisionCache` + `route_health` + `domain_conn_caps` + failover 状态。
