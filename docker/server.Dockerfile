...
# RinaDown headless 服务器镜像：单二进制 rinadown-server（Web SPA 编译期内嵌）。
...
#   docker build -f docker/server.Dockerfile -t rinadown-server .
...
#   docker build -f docker/server.Dockerfile --build-arg RINADOWN_SERVER_VERSION=1.2.3 .
#
# 运行（首次打开 Web 界面时自行设置访问密钥；也可用 -e RINADOWN_TOKEN=... 预置）：
#   docker run -d -p 17800:17800 -v rinadown-data:/data rinadown-server
...
# ── Stage 2: Rust 服务器（workspace 成员，仅编译 rinadown_server，按 TARGETARCH 交叉编译）──
...
ENV RINADOWN_EMBED_WEBROOT=/webroot
# 编译期版本注入（空值 = 未注入，二进制退回 crate 版本）
ARG RINADOWN_SERVER_VERSION
ENV RINADOWN_SERVER_VERSION=$RINADOWN_SERVER_VERSION
# 编译期匿名统计 App-Key 注入（空值 = 未注入，统计整体禁用）
ARG RINADOWN_ANALYTICS_APP_KEY
ENV RINADOWN_ANALYTICS_APP_KEY=$RINADOWN_ANALYTICS_APP_KEY
...
RUN --mount=type=cache,id=rinadown-cargo-registry-$TARGETARCH,target=/usr/local/cargo/registry,sharing=locked \
    --mount=type=cache,id=rinadown-cargo-target-$TARGETARCH,target=/src/target,sharing=locked \
    cargo build --release --locked -p rinadown_server --target "$(cat /rust-target)" \
    && cp "target/$(cat /rust-target)/release/rinadown-server" /usr/local/bin/rinadown-server
...
RUN apt-get update \
    ...
    && rm -rf /var/lib/apt/lists/*
WORKDIR /app
COPY --from=server /usr/local/bin/rinadown-server /app/rinadown-server
# RINADOWN_BIND / RINADOWN_DATABASE_URL / RINADOWN_DEMO / RINADOWN_LANG 等见 native/server/src/config.rs
ENV RINADOWN_BIND=0.0.0.0:17800 \
    RINADOWN_DATA_DIR=/data
VOLUME /data
EXPOSE 17800
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s \
    CMD curl -fsS "http://127.0.0.1:${RINADOWN_BIND##*:}/ping" || exit 1
ENTRYPOINT ["/app/rinadown-server"]
