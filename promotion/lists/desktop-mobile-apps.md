...
- 仓库 `zerx-lab/RinaDown`，AGPL-3.0，1002 star，主语言 Rust（GitHub 判定）。
- 官网 `https://rinadown.zerx.dev`（README 全文使用此域名）。
- ⚠️ **GitHub 仓库 Homepage 字段目前填的是 `https://www.rinadown.com`，与 README 不一致**。提交前务必统一，否则审核者点开会看到两个不同官网 → 见「跨清单前置项」。
- 浏览器扩展**已上架三家商店**（Chrome Web Store / Edge Add-ons / Firefox AMO），扩展类清单可用。
- Android：已发 per-ABI + universal APK（`com.rinadown.app`），**未上架 F-Droid / IzzyOnDroid / Google Play**。
- Android 端依赖已核查：`android/` 与 `pubspec.yaml` 中**无** Firebase / play-services / Crashlytics / 友盟 / Bugly 等专有 SDK → 满足 android-foss「无专有成分」要求。
- Windows 包管理：仓库内有自建 Scoop manifest（`bucket/rinadown.json`），**未提交到 ScoopInstaller/Extras**；**无 winget、无 Homebrew Cask**。
...
1. 一个 PR 只加一个应用；PR 标题写成 `Add RinaDown to Download Management Tools`（照抄现有 PR 命名习惯，如 #2396 `Add Berthly to Virtualization`）。
2. **必须四语言同步**：`README.md`、`README-zh.md`、`README-ja.md`、`README-ko.md` 四个文件都要改，缺一会被打回。
3. 描述**限一句话**，句首大写，条目名用 title case。
4. 章节内**按字母序**插入。
5. 图标是引用式链接（文件底部已定义），RinaDown 属「开源 + 免费」→ 用 `[![Open-Source Software][OSS Icon]](<源码地址>) ![Freeware][Freeware Icon]`。
...
```markdown
* [RinaDown](https://rinadown.zerx.dev) - Multi-protocol download manager with IDM-style dynamic segmentation for HTTP, FTP, BitTorrent, eD2K, HLS and DASH. [![Open-Source Software][OSS Icon]](https://github.com/zerx-lab/RinaDown) ![Freeware][Freeware Icon]
```
...
```markdown
* [RinaDown](https://rinadown.zerx.dev) - 支持 HTTP、FTP、BitTorrent、eD2K、HLS 与 DASH 的多协议下载管理器，具备 IDM 式动态分段加速。 [![Open-Source Software][OSS Icon]](https://github.com/zerx-lab/RinaDown) ![Freeware][Freeware Icon]
```
...
```markdown
* [RinaDown](https://rinadown.zerx.dev) - HTTP、FTP、BitTorrent、eD2K、HLS、DASHに対応し、IDM風の動的セグメント分割を備えたマルチプロトコル・ダウンロードマネージャー。 [![Open-Source Software][OSS Icon]](https://github.com/zerx-lab/RinaDown) ![Freeware][Freeware Icon]
```
...
```markdown
* [RinaDown](https://rinadown.zerx.dev) - HTTP, FTP, BitTorrent, eD2K, HLS, DASH를 지원하고 IDM 방식의 동적 세그먼트 분할을 갖춘 멀티 프로토콜 다운로드 관리자. [![Open-Source Software][OSS Icon]](https://github.com/zerx-lab/RinaDown) ![Freeware][Freeware Icon]
```
...
### 准入 criteria（逐条比对 RinaDown）

| criteria | RinaDown | 说明 |
|---|---|---|
| FOSS 许可 | ✅ | AGPL-3.0 |
| 源码可获取 | ✅ | 公开仓库 |
...
```markdown
* [**RinaDown**](https://github.com/zerx-lab/RinaDown)
```

> 现在**不能**加 `<sup>**[[F-Droid](...)]**</sup>` 或 IzzyOnDroid 角标——RinaDown 两处都未上架，写了就是造假。
>
...
> `* [**RinaDown**](https://github.com/zerx-lab/RinaDown) <sup>**[[IzzyOnDroid](https://apt.izzysoft.de/packages/com.rinadown.app)]**</sup>`
...
3. 描述限一句；语言列 = GitHub 判定的主语言（RinaDown = `Rust`）；license 列用 SPDX。
4. star 列 ≥1000 写 `1.0k` 形式（会被脚本自动刷新）。
5. Download 列可以是 Google Play / F-Droid 徽章、releases 链接，或 `—`。RinaDown 用 Releases 徽章（现网先例：AIMSICD、PocketPal AI）。
...
```markdown
| [**RinaDown**](https://github.com/zerx-lab/RinaDown) | A multi-protocol download manager with IDM-style dynamic segmentation for HTTP, FTP, BitTorrent, eD2K, HLS and DASH. | `Rust` | `AGPL-3.0` | 1.0k | [![Download](https://img.shields.io/badge/Download-Releases-blue)](https://github.com/zerx-lab/RinaDown/releases) |
```
...
2. **字母序**。
3. `[oss icon]` 表示开源并链接到源码；`[money icon]` 表示收费。RinaDown 免费开源 → 只用 `oss icon`。
...
```markdown
- [![Open-Source Software][oss icon]](https://github.com/zerx-lab/RinaDown) [RinaDown](https://rinadown.zerx.dev/) - RinaDown is a Rust-powered multi-protocol download manager with IDM-style dynamic segmentation, supporting HTTP/HTTPS, FTP, BitTorrent, eD2K, HLS and DASH.
```
...
**够格但不理想**：该清单**没有**下载类章节（目录里只有 Audio / Backup / Chat Clients / Editors / Finder / Sharing Files / Utilities / Video / Window Management 等），RinaDown 只能进 `### Utilities`。
...
- ⛔ 禁止：让任何 agent 代为发 PR、代写 PR 描述、或在 PR 中留下 AI 痕迹。PR 描述请自己写一两句人话说明 RinaDown 是什么、为什么值得收录。
...
```markdown
* [RinaDown](https://rinadown.zerx.dev) - Multi-protocol download manager with IDM-style dynamic segmentation for HTTP, FTP, BitTorrent and HLS/DASH streams. [![Open-Source Software][oss]](https://github.com/zerx-lab/RinaDown)
```
...
```markdown
- [RinaDown](https://rinadown.zerx.dev) - Multi-protocol download manager with IDM-style dynamic segmentation for HTTP, FTP, BitTorrent, eD2K, HLS and DASH. [![Open-Source Software][oss icon]](https://github.com/zerx-lab/RinaDown) ![Freeware][freeware icon] ![Freeware][freeware icon light]
```
...
调研结论：**不存在**面向普通用户的、活跃的通用「awesome browser extensions」清单（详见下文「已死 / 不够格」）。本仓库是唯一可用的落点，且 RinaDown 扩展**已上架 Chrome Web Store**，满足前置条件。
...
3. **扩展必须在最近 6 个月内有更新**——RinaDown 持续发版，满足；提交前确认 Chrome Web Store 页面的 "Updated" 日期在 6 个月内。
...
```markdown
| 9 | [**RinaDown - Download Manager Integration**](https://chromewebstore.google.com/detail/rinadown/meleenglfggcmcajknpeeeiobnpfmahc) | Sends browser downloads and sniffed HLS/DASH video streams to the RinaDown desktop app for multi-threaded, resumable, segmented downloading. |
```
...
回报很低，但 `## Downloader` 章节**目前只有 Internet Download Manager 一条**——RinaDown 定位正是 IDM 开源替代，插进去语义完美，几乎不可能被拒。5 分钟成本，顺手做掉。
...
```markdown
* [RinaDown](https://rinadown.zerx.dev) - Free and open-source multi-protocol download manager with IDM-style dynamic segmentation, supports HTTP/FTP/BitTorrent/eD2K/HLS/DASH.
```
...
| `open-saas-directory/awesome-native-macosx-apps` | **不够格** | 准入条件明写「✅ 用 Swift/SwiftUI/AppKit/Objective-C 构建」「❌ 不收 Electron 或 web wrapper」「❌ 不收资源占用高的跨平台应用」。RinaDown 是 Flutter UI，属被排除的跨平台桌面应用，提了会被拒。 |
| `herrbischoff/awesome-macos-command-line` | **已归档** | 且只收命令行工具。 |
| `stefanbuck/awesome-browser-extensions-for-github` | **不够格** | 3.3k star 且活跃，但只收「用于 GitHub 网站的扩展」。RinaDown 扩展与 GitHub 无关。 |
...
| `kklt92/awesome-ai-extensions` | **不够格** | 只收 AI 驱动的扩展。RinaDown 扩展本身不是 AI 扩展（内置 MCP server 在桌面端，不在扩展里）。 |
...
1. **【阻塞 · 必做】统一官网 URL。** GitHub 仓库 Homepage 字段是 `https://www.rinadown.com`，README 通篇是 `https://rinadown.zerx.dev`。所有清单条目都要填官网，审核者一定会点开；两个域名不一致会直接引发「这项目到底哪个是官网」的质疑。**先决定唯一 canonical 域名，改掉仓库 Homepage 字段**，再按最终结果批量替换本文件里所有条目文本中的 `https://rinadown.zerx.dev`。
...
5. **【非阻塞】** 所有清单均**无 star 门槛、无项目年龄门槛、无截图或演示站要求**。RinaDown 现有 1002 star 远超一般心理门槛，无需额外准备。
...
均以 `zerx-lab` 身份提交：`gh repo fork` → 分支 `add-rinadown` → 单文件最小改动 → `gh pr create`。所有链接统一 `https://rinadown.zerx.dev`，无 AI 署名 / Co-authored-by / emoji 标记。
