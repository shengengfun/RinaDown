...
任务书写的「RinaDown 镜像目前仅 linux/amd64」**与事实不符**。实测证据：
...
| GHCR manifest list（匿名 token 直查 `ghcr.io/v2/zerx-lab/rinadown-server/manifests/<tag>`） | `latest`、`0.1.59`、`0.2.0`、`0.2.2`、`0.2.3`、`0.2.5-rc.2` **全部返回含 `linux/amd64` + `linux/arm64` 的 OCI index** |
...
| 最早的 **git tag** | `v0.0.1`，commit date **2026-02-10** | `GET /repos/zerx-lab/RinaDown/tags`（共 131 个 tag） |
| 最早的 **GitHub Release** | `website-v0.1.49` / `v0.1.49`，**2026-07-03** | `GET /repos/zerx-lab/RinaDown/releases?per_page=100 --paginate`（共 80 条） |
...
- **B**：不改仓库，在 PR 正文显式写明并给出可点击证据：
  > First public release is tag [`v0.0.1`](https://github.com/zerx-lab/RinaDown/releases/tag/v0.0.1), dated 2026-02-10 (5.5 months ago). Older tags predate our switch to GitHub Releases; the full tag history is at https://github.com/zerx-lab/RinaDown/tags.
...
| 清单仓库 | star 量级 | 最近活跃（最近一次合并 PR / push） | RinaDown 是否够格 | 是否禁 AI-PR | 优先级 |
|---|---|---|---|---|---|
...
```yaml
name: RinaDown
website_url: https://rinadown.zerx.dev
...
source_code_url: https://github.com/zerx-lab/RinaDown
```

**相对现有 `promotion/awesome-selfhosted/rinadown.yml` 的改动及理由**：
...
**`tags` 取值校验**：`tags/file-transfer---peer-to-peer-filesharing.yml` 内 `name:` 字段实测为 `File Transfer - Peer-to-peer Filesharing`，与上面一字不差。全仓 95 个 tag 中**没有** "Download manager" 类目；候选替代是 `Media Management`（MeTube 用的）与 `Automation`（pyLoad 用的）。RinaDown 有 BitTorrent + eD2K/Kad，P2P 标签最贴切，且与 Transmission 同类目。**注意**：single page mode 下只会显示 `tags` 列表里的**第一个**类目，所以第一个必须是 P2P。

### 插入位置

新建独立文件，不涉及字母序插入。文件名 `rinadown.yml` 决定排序。
...
|---|---|
| 一个 PR 只提一个条目 | ✅ |
| 已搜索过 issue / PR（含已关闭） | ✅ 实测 `repo:awesome-selfhosted/awesome-selfhosted-data RinaDown` → 0 结果，无历史投稿 |
| 未被 awesome-sysadmin / staticgen / dbdb.io 收录 | ✅ |
| 文件格式符合 `addition.md` | ✅ 见上 |
| 使用 kebab-case 文件名 | ✅ `rinadown.yml` |
...
CONTRIBUTING → *What does not qualify* 中有两条正对 RinaDown：
...
1. > Software that is a **desktop**, mobile, or command-line application, which relies on a separate file synchronisation/server program
   ...
   RinaDown 主线是 Flutter 桌面应用。**PR 必须把主体定位在 `rinadown_server`**：一个 headless 的 Rust 服务端，自带 Web UI（端口 17800），可独立部署、无需桌面客户端。桌面 App 是可选客户端而非前提。

2. > Software contributions that merely **port an existing application to another system** (e.g., Dockerization)

   不适用 —— RinaDown 是自研引擎，非既有应用的容器化封装。可主动说明。

**建议的 PR 正文骨架（人类改写后使用，勿直接粘贴）**：
> RinaDown ships a headless server binary (`rinadown_server`) with a built-in web UI on port 17800, packaged as a multi-arch image (`ghcr.io/zerx-lab/rinadown-server`, linux/amd64 + linux/arm64). It runs standalone on a NAS or VPS — the desktop app is an optional client, not a requirement. The download engine is written from scratch in Rust/Tokio, not a wrapper around an existing downloader.
...
- commit message 用 `add RinaDown`，勾选 *Create a new branch for this commit and start a pull request*。
...
```yaml
- name: RinaDown
  url: 'https://github.com/zerx-lab/RinaDown'
```
...
| 路线 | 做法 | 适用 |
|---|---|---|
| **A（推荐）** | 往 `sources/local/` 丢一个 JSON 文件 | 只有一两个模板 —— 正是 RinaDown 的情况 |
...
```
sources/local/rinadown_templates.json
```
  ...
  "templates": [
    {
      ...
      "platform": "linux",
      "logo": "https://cdn.jsdelivr.net/gh/zerx-lab/RinaDown@main/assets/logo/rinadown_logo.svg",
      "image": "ghcr.io/zerx-lab/rinadown-server:0.2.3",
...
| star / 年龄门槛 | 无 |
| 截图 / 演示站 | 无 |
| **必须公开 Docker 镜像** | ✅ **是硬性的** —— `type: 1` 模板必填 `image`。RinaDown 有 `ghcr.io/zerx-lab/rinadown-server`，满足。 |
...
页面自述：*This is a community discovery page, not a formal endorsement list.* —— 门槛低，但只收**整个 store**，不收单个 app。所以 RinaDown 必须先把 `promotion/casaos/` 发布成一个可访问的 store。

### 前置：先修 `promotion/casaos/Apps/RinaDown/docker-compose.yml`
...
| `services.rinadown-server.image` | `ghcr.io/zerx-lab/rinadown-server:0.1.54` | `ghcr.io/zerx-lab/rinadown-server:0.2.3` | 最新稳定版为 `v0.2.3`（2026-07-17）；`0.2.4` 无稳定发布，`0.2.5-rc.2` 是预发布。同步改 `x-casaos.version: "0.2.3"` 与 `x-casaos.update_at: "2026-07-17"`。 |

其余字段实测已满足 v2 规范 *First app checklist*（顶层 compose `name`、`services`、`x-casaos.id`/`main`/`index`/`port_map`/`icon`/`title`/`category` 全部齐备，`port_map` 已正确写成带引号字符串 `"17800"`，`id` 为合法反向域名 `dev.zerx.rinadown`）。
...
1. 把 promotion/casaos/ 作为独立仓库（或 GitHub Pages 分支）的根：
   my-appstore/
   ├── Apps/RinaDown/docker-compose.yml
   ├── Apps/RinaDown/icon.svg          ← 建议补上本地图标
...
```mdx
<StoreSourceCard
  title="RinaDown Store"
  summary="Official RinaDown multi-protocol download manager for CasaOS and ZimaOS."
  url="https://cdn.jsdelivr.net/gh/zerx-lab/rinadown-appstore@gh-pages/store.json"
  maintainer="zerx-lab"
  repo-url="https://github.com/zerx-lab/rinadown-appstore"
/>
```
...
| 必须 arm64 | **非硬性**，但 `architectures` 必须如实声明；RinaDown 已支持 arm64，如实写上是净收益 |
| 禁 AI PR | 无禁令 |
...
```
App request: RinaDown

Repo:     https://github.com/zerx-lab/RinaDown
Website:  https://rinadown.zerx.dev
License:  AGPL-3.0
Image:    ghcr.io/zerx-lab/rinadown-server:0.2.3
...
is available at:
https://github.com/zerx-lab/RinaDown/blob/main/promotion/casaos/Apps/RinaDown/docker-compose.yml
```
...
**物料已就绪**，见 `promotion/unraid/`（`ca_profile.xml` + `RinaDown/rinadown.xml` + 说明 README）。该目录已正确判断出「必须作为独立仓库 `zerx-lab/unraid-templates` 的根推送」，本节只补充**提交流程与当前硬性要求**，不重复其内容。
...
`promotion/unraid/RinaDown/rinadown.xml` 里的架构声明未在本次调研中逐字段核对。Unraid 仅支持 x86_64，所以 arm64 在此**无关紧要**；但若模板中有 `<Requires>` 或架构相关字段，应与 amd64 一致。**标注：未核实。**
...
categories:
- media
changelog_url: https://github.com/zerx-lab/RinaDown/releases
date_added: '2026-07-27'
description: RinaDown is a multi-protocol download manager with a web UI, supporting
  HTTP/FTP/BitTorrent/eD2K/HLS/DASH with dynamic segmentation and crash-safe resume.
home: https://rinadown.zerx.dev/
host_mounts: []
icon: https://media.sys.truenas.net/apps/rinadown/icons/icon.png
keywords:
- media
- torrent
- download
...
maintainers:
- email: contact@rinadown.app
  name: zerx-lab
  url: https://github.com/zerx-lab
name: rinadown
run_as_context:
- description: Container [rinadown] runs as root inside the container.
  gid: 0
  group_name: root
  uid: 0
  user_name: root
screenshots: []
sources:
- https://github.com/zerx-lab/RinaDown
...
- `run_as_context` 需与实际容器用户一致。RinaDown 服务端镜像默认以 root 运行（compose 里挂载 `/root/Downloads` 可佐证）。若后续改为非 root，此处需同步。**当前镜像内用户身份未逐字节核实 —— 标注：未核实**，提交前应 `docker run --rm --entrypoint id ghcr.io/zerx-lab/rinadown-server:0.2.3` 确认。
...
**处置**：先写一篇 RinaDown 的 NAS/Docker 部署教程（知乎 / 什么值得买 / 个人博客均可），拿到 URL 再投。
...
分支 `main`。该文件对应 README 的「基础设施 —— 存储方案、网络服务、消息队列、VPN代理」类目，现网 Gopeed（全平台多协议高速下载工具）与 Exatorrent 都在此文件内，是 RinaDown 最贴切的落点。
...
```markdown
| RinaDown | Rust引擎驱动的多协议下载器，支持BT/eD2K/HLS | [点我查看](https://github.com/zerx-lab/RinaDown) | [查看教程](https://zhuanlan.zhihu.com/p/替换为你的教程ID) | ![Star](https://img.shields.io/github/stars/zerx-lab/RinaDown?&label=) | ![Last Commit](https://img.shields.io/github/last-commit/zerx-lab/RinaDown?label) |
```
...
|---|---|
| **配套教程文章** | ⛔ **事实上硬性**（见上） |
| 必须公开 Docker 镜像 | 事实上是（清单主题即 Docker 部署），RinaDown 满足 |
| 必须 arm64 | 无此要求 |
| star / 年龄 / 演示站 | 无明文门槛 |
| 禁 AI PR | 无禁令 |
...
```markdown
| RinaDown | 下载 | Rust引擎驱动的多协议下载器，支持HTTP/BT/eD2K/HLS | [项目地址](https://github.com/zerx-lab/RinaDown) | [教程地址](https://替换为你的教程URL) | ⭐New！ |
```
...
清单活跃（push 2026-07-22），但 `.github/CONTRIBUTING.md` 设了一道专门排除 RinaDown 这类项目的准入测试：
...
RinaDown 的填空是「下载文件」，不含任何 Docker 关键词。**结论：结构性不够格，提交必被拒，且会浪费维护者时间。放弃。**
...
（补充：RinaDown 确实产出 `.ipk`，OpenWrt 方向的真实渠道是官方 `openwrt/packages` 软件源，那是**包管理提交**而非清单收录，不属于本 slice 范围。**未核实**其准入细则。）
...
LinuxServer.io 不维护「收录清单」。他们**自己构建并维护全部镜像**（`linuxserver/docker-<app>` 系列），第三方镜像无法被「收录」—— 只能请求他们**从零为 RinaDown 新建一个由他们维护的镜像仓库**，那是把镜像所有权交出去，与 RinaDown 自建 GHCR 多架构镜像的现状冲突。`linuxserver/docker-mods` (1.5k star) 是给现有 LSIO 镜像打补丁的机制，与本项目无关。

**结论：不适用。** 若确实想借其渠道，可考虑的是 `technorabilia/portainer-templates`（230 star，专门收 LSIO 系模板）—— 但同样只收 LSIO 镜像，RinaDown 不符。
...
TOKEN=$(curl -s "https://ghcr.io/token?scope=repository:zerx-lab/rinadown-server:pull&service=ghcr.io" | jq -r .token)
curl -s -H "Authorization: Bearer $TOKEN" \
     -H "Accept: application/vnd.oci.image.index.v1+json" \
     https://ghcr.io/v2/zerx-lab/rinadown-server/manifests/latest \
...
gh api "repos/zerx-lab/RinaDown/releases?per_page=100" --paginate \
   -q '.[] | "\(.published_at) \(.tag_name)"' | sort | head -1
# → 2026-07-03T03:12:17Z website-v0.1.49

# 3. 验证最早的 tag
gh api "repos/zerx-lab/RinaDown/tags?per_page=100" --paginate -q '.[].name' | tail -1   # v0.0.1
gh api repos/zerx-lab/RinaDown/commits/v0.0.1 -q .commit.committer.date                 # 2026-02-10

# 4. 验证最新稳定版
gh api repos/zerx-lab/RinaDown/releases/latest -q .tag_name    # v0.2.3
...
| 目标 | 结果 | URL | 备注 |
|---|---|---|---|
| AwesomeHomelab/awesome-homelab | ✅ 已提 PR，OPEN | <https://github.com/AwesomeHomelab/awesome-homelab/pull/107> | 「Add RinaDown to Download category」，早于本轮完成 |
| lissy93/portainer-templates | ✅ 已提 PR，OPEN | <https://github.com/lissy93/portainer-templates/pull/123> | 新增 `sources/local/rinadown_templates.json` |
| selfhosters/unRAID-CA-templates | ✅ 已提 PR，OPEN | <https://github.com/selfhosters/unRAID-CA-templates/pull/686> | 新增 `templates/rinadown.xml` |
...
- 分支 `zerx-lab:add-rinadown-template`，仅新增 1 个文件，未碰 `templates.json`（自动生成物）。
...
- **本地校验已跑通**（Windows 无系统 python，用 `uv` 拉临时环境）：
  ...
  PYTHONPATH=lib uv run --with jsonschema --with pyyaml --with requests \
    python lib/validate_sources.py sources/local/rinadown_templates.json
  # → Checked sources\local\rinadown_templates.json: 0 errors, 0 warnings
  ```
...
- `<Icon>` 改用 **PNG**（`raw.githubusercontent.com/zerx-lab/RinaDown/main/assets/logo/rinadown_logo.png`，HTTP 200 已验），不用 `promotion/unraid/` 里那份 SVG —— 正是 `promotion/unraid/README.md` 自己提的改进项。
- `<Repository>` 用 `ghcr.io/zerx-lab/rinadown-server:latest`（**不是**物料里的固定版本号）。CA 靠 tag 的 digest 变化判断「有更新」，钉死版本号等于永远不提示升级；该仓其余模板也一律用 `:latest`。`latest` 的 GHCR manifest 已验为 `linux/amd64` + `linux/arm64`。
- `<TemplateURL>` 指向 **selfhosters 仓库**的 raw 路径（不是自有仓库），否则 CA 会去拉错源。
- 本地校验：`uv run python .github/scripts/check.py --files templates/rinadown.xml` → **零输出**（无 error、无 notice）；XML 解析通过。
...
RinaDown 目前**没有已发布的 store**，`promotion/casaos/` 只是一份未构建、未托管的源目录。现在提 PR 只能填一个填不出来的 `url`，必被驳回。
...
2. **补 `Apps/RinaDown/icon.svg`** —— 同一份 *Minimal source structure* 里列出的本地图标。
3. **修 `promotion/casaos/Apps/RinaDown/docker-compose.yml`** —— 见上方「P1 — CasaOS / ZimaOS 第三方源清单 → 前置」：`x-casaos.architectures` 补 `arm64`（**不要写 armv7**，GHCR 无 `linux/arm/v7`），`image` 从 `0.1.54` 升到 `0.2.3`，同步改 `x-casaos.version` 与 `update_at`。
4. **建独立仓库并发布 dist** —— 把 `promotion/casaos/` 作为某个仓库（建议 `zerx-lab/rinadown-appstore`）的根，跑 `./scripts/build_dist.sh` 或官方 `IceWhaleTech/build-appstore-action`，把 `dist/` 发到 GitHub Pages / Cloudflare Pages。
...
- 历史：<https://github.com/awesome-selfhosted/awesome-selfhosted-data/pull/2675>「Add RinaDown」，**2026-07-05 08:24 UTC 被关闭**，维护者 Rabenherz112 的理由是「Initial release 2 days ago」——即 CONTRIBUTING 的「首个 release 须早于 4 个月前」。
...
- 重提时：条目 YAML 直接用上方「P0 — awesome-selfhosted」章节的版本（已校过 `tags` / `platforms` 取值），PR 正文务必由人类撰写（该仓 CONTRIBUTING 明文「Machine/LLM-generated contributions … will result in a ban」），并按「不够格风险点」一节把主体定位在 headless 的 `rinadown_server` 上。
