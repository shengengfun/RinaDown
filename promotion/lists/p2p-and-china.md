# RinaDown 收录物料：下载/P2P 垂直清单 + 中文开源社区渠道
...
`zerx-lab/RinaDown` 仓库的 **Homepage 字段填的是 `https://www.rinadown.com`**，而 README.md:19 / README.zh-CN.md:19 里的官网全是 `https://rinadown.zerx.dev`。
多数清单维护者会点仓库右上角的 Homepage 做核对，两个域名不一致会被当成可疑条目。**提交任何清单前先把 Homepage 统一成 `https://rinadown.zerx.dev`**（或反过来，取一个作为唯一官网）。

本文件所有条目文本统一使用 `https://rinadown.zerx.dev`。
...
这一节决定「RinaDown 能不能自称 aria2 兼容」。结论：**能，而且是全方法集覆盖，但有 5 个方法明确拒绝、4 个方法降级**。以下全部有代码出处，不夸大。

### 2.1 实现位置

| 文件 | 职责 |
|---|---|
| `RinaDown/native/api/src/jsonrpc.rs` | `POST /jsonrpc` 派发层（`dispatch_method`，L137–180） |
| `RinaDown/native/api/src/aria2.rs` | 纯函数映射层：GID 编解码、status 映射、选项翻译、响应拼装 |
| `RinaDown/native/api/src/jsonrpc_ws.rs` | `GET /jsonrpc` WebSocket 升级 + aria2 风格通知广播 |
| `RinaDown/native/api/src/server.rs` | 路由注册；`jsonrpc_enabled` **默认 true**（server.rs:69, :82–83） |
| `RinaDown/native/api/src/routes.rs` | `pub const JSONRPC: &str = "/jsonrpc"`（routes.rs:23） |
| `RinaDown/native/api/src/tests.rs` | 真实 TCP + `serve_on` 的黑盒集成测试（L660+ 为 aria2 端点专章） |
...
- 鉴权：`X-RinaDown-Token` 头 或 aria2 约定的 `params[0] = "token:xxx"`（`jsonrpc.rs:73`、`strip_token_prefix` L128）。`system.listMethods` / `system.listNotifications` **不鉴权**，对齐 aria2 重写 `execute()` 的行为（`jsonrpc.rs:99`）。`system.multicall` 信封本身不鉴权，token 由每个子调用各自携带（`jsonrpc.rs:106`、L213–216）。
...
| AriaNg 拖拽调序 | `changePosition` | **被拒绝** → 该功能在 RinaDown 后端上不可用 |
...
- **目标文件**：`README.md`（主版面。子版面 `pages/README-Programmer-Edition.md` 是「需要命令行或写代码」的产品；RinaDown 桌面端是打开即用的 GUI App，**走主版面**）
- **插入位置**：`## 3. 项目列表` 之下、现有最新日期小节 **之上**，新建一个当天日期的 `###` 小节。该列表按「添加日期倒序」排，不是字母序。
- **CONTRIBUTING 硬性要求**（来自 `README.md` 顶部 + `CONTRIBUTING.md`）：
  - 入选标准：**必须是网站或 App，不能是开发者工具或论坛型网站** → RinaDown 是桌面/移动 App，符合。
  - 无 star 门槛、无项目年龄门槛、无截图要求。
  - 状态 emoji 必须选一个：`:clock8:` 开发中 / `:white_check_mark:` 已上线 / `:x:` 已关闭。RinaDown 已发布多个 release → `:white_check_mark:`。
...
* :white_check_mark: [RinaDown](https://rinadown.zerx.dev)：多协议下载管理器，Rust 引擎 + IDM 式动态分段，支持 HTTP/FTP/BT 磁力/eD2K/HLS/DASH，浏览器扩展自动接管下载，Win/macOS/Linux/Android 全平台开源免费
```

> 日期小节按实际提交日改。若当天已存在同日期 `###` 小节，则**不新建小节**，直接在该小节末尾追加 `####` 作者块。
> 提交 PR 的标题照抄社区惯例：`添加 RinaDown` 或 `自荐:RinaDown(开源多协议下载管理器)`。
...
- **额外审核标准**（模板尾部指向的 [issue #271](https://github.com/521xueweihan/HelloGitHub/issues/271)，2018 年开帖，2026-03 仍在被引用）：
  - 文档必须含：项目介绍（必须）、特性（必须）、快速开始（必须）、版权/协议（必须）；ChangeLog 对**自荐项目可选**。
    → 实测 `RinaDown/README.md` 已有 `## Highlights`(L27) / `## Features`(L37) / `## Installation`(L65) / `## License`(L214)，**四项必须全部满足**。
  - 自荐项目 star 数是**可选**参考项（RinaDown 现 1002★，够用）。
  - Insights 活跃度是**必选**参考项（自荐也逃不掉）→ 提交前保证近 30 天有稳定 commit。
  - 「同类型项目数量过多则不予推荐」→ **亮点字段必须打出差异化**，别写成又一个下载器。
  - **请勿使用复制的内容作为项目描述**（照搬 README 会被判定复制）。
  - 提交前先在 https://hellogithub.com 搜 `RinaDown` 确认未被收录过。
- **可直接复制粘贴的字段内容**：
...
```
RinaDown 用 Rust + Tokio 写下载引擎、Flutter 做界面，把 HTTP/FTP、BitTorrent 磁力、eD2K/Kad、HLS 与 DASH 收进同一个任务队列。它做 IDM 式动态分段：不是开固定线程数，而是在下载过程中把还没下完的区间继续切开分给空闲连接，慢速节点不会拖住整条任务。装上浏览器扩展后网页里的下载会被自动接管，断点续传落在 SQLite WAL 上，杀进程也不丢进度。
```
...
5. 同一套代码跑桌面（Win/macOS/Linux）、Android 和 headless 服务端（rinadown_server + Web UI，支持群晖 spk / QNAP qpkg / OpenWrt ipk / Unraid / CasaOS）。
```

**截图**：直接拖 3 张 —— 主界面（含分段进度条）、亮/暗主题对比、AriaNg 连上 RinaDown 的截图（第 3 张是最有说服力的差异化证据）。
...
- **标题格式**（实测 2026-07-25/26 的 12 条投稿，压倒性使用这两种前缀）：
  ...
  RinaDown 是 AGPL-3.0 开源 → 用 `【开源自荐】`。
- **可直接复制粘贴的 issue 标题**：

```
【开源自荐】RinaDown：Rust 引擎的多协议下载器，IDM 式动态分段 + aria2 RPC 兼容
```

- **可直接复制粘贴的 issue 正文**：

```markdown
仓库：https://github.com/zerx-lab/RinaDown
官网：https://rinadown.zerx.dev
协议：AGPL-3.0

RinaDown 是一个多协议下载管理器，Rust + Tokio 写引擎、Flutter 做界面，目标是做 IDM 的开源替代。
...
- **内置 aria2 JSON-RPC 兼容层**。覆盖 aria2 官方 36 个方法全集，其中 27 个是真实映射到引擎的实现（另有 4 个降级返回、5 个明确拒绝，比如 changePosition 和 shutdown）。所以 AriaNg 和各种「发送到 aria2」的油猴脚本可以直接把 RinaDown 当后端用。
...
另有 headless 服务端 `rinadown_server`（Web UI 端口 17800，镜像 `ghcr.io/zerx-lab/rinadown-server`，amd64 + arm64 多架构），支持群晖 spk / QNAP qpkg / OpenWrt ipk / Unraid / CasaOS。
```
...
- **CONTRIBUTING 硬性要求**：
  ...
  - Commit message 必须是 `Add: RinaDown`（`Add:` / `Update:` / `Remove:` 三选一）。
...
```markdown
- [RinaDown](https://rinadown.zerx.dev) - Multi-protocol download manager with IDM-style dynamic segmentation, supporting HTTP, FTP, BitTorrent, eD2K, HLS and DASH. 🪟 🍎 🐧 [🟢](https://github.com/zerx-lab/RinaDown)
```
...
```markdown
- [RinaDown](https://rinadown.zerx.dev) - Multi-protocol download manager with IDM-style dynamic segmentation. 🤖 [🟢](https://github.com/zerx-lab/RinaDown)
```
...
- **CONTRIBUTING 硬性要求**（`.github/CONTRIBUTING.md`）：
  ...
  - 提交前必须先在 https://fmhy.net/single-page.md 用 `ctrl+f` 搜 `RinaDown` 确认未收录。
  - 不收：付费 / 仅试用产品（RinaDown 免费开源，✅）、非英文软件（除非口碑极好；RinaDown 有英文 README + 英文 UI，✅）。
...
```markdown
* [RinaDown](https://rinadown.zerx.dev/) - Download Manager / [Firefox](https://addons.mozilla.org/firefox/addon/rinadown) / [Chrome](https://chromewebstore.google.com/detail/rinadown/meleenglfggcmcajknpeeeiobnpfmahc) / [Edge](https://microsoftedge.microsoft.com/addons/detail/rinadown/nglkkjbogjghekbhhcnccnpfedjbdhhd) / Windows, macOS, Linux, Android / [GitHub](https://github.com/zerx-lab/RinaDown)
```
...
- **诚实评估**：这个清单的定位是「免费下载油管、B 站、抖音等平台视频的工具」，主体是站点抽取器（lux / BBDown / downkyi / N_m3u8DL-CLI）。RinaDown **没有站点抽取器**，只有 HLS/DASH 直链下载能力。**唯一的先例是 gopeed —— 一个同样没有站点抽取器的通用下载管理器已被收录在「最受欢迎的」表里**，所以并非完全不够格，但被拒的概率不低。
...
```markdown
![RinaDown](https://socialify.git.ci/zerx-lab/RinaDown/image?description=1&forks=1&issues=1&language=1&name=1&owner=1&pulls=1&stargazers=1&theme=Light)![img](https://img.shields.io/github/stars/zerx-lab/RinaDown?label=Star)</br>[RinaDown](https://github.com/zerx-lab/RinaDown)
```

即把最后一行改成：

```markdown
|![wechatVideoDownload](https://socialify.git.ci/qiye45/wechatVideoDownload/image?description=1&forks=1&issues=1&language=1&name=1&owner=1&pulls=1&stargazers=1&theme=Light)![img](https://img.shields.io/github/stars/qiye45/wechatVideoDownload?label=Star)</br>[wechatVideoDownload](https://github.com/qiye45/wechatVideoDownload)|![RinaDown](https://socialify.git.ci/zerx-lab/RinaDown/image?description=1&forks=1&issues=1&language=1&name=1&owner=1&pulls=1&stargazers=1&theme=Light)![img](https://img.shields.io/github/stars/zerx-lab/RinaDown?label=Star)</br>[RinaDown](https://github.com/zerx-lab/RinaDown)|
```
...
- **结论**：**没有任何「外部下载器清单」文件可以提 PR 加一行**。想被 yt-dlp 支持，只能提交一个 `RinaDownFD(ExternalFD)` 类的代码 PR。
...
- **技术前提（本仓库已具备的部分）**：
  - CLI 二进制名是 `rinadown`（`RinaDown/native/cli/Cargo.toml` 的 `[[bin]] name = "rinadown"`）；
  - CLI 已对齐 aria2c 的退出码语义（`RinaDown/native/cli/src/exit.rs:1`「对齐 aria2c 的退出码语义（`man aria2c` EXIT STATUS）」），这是 `ExternalFD` 判断成败所依赖的东西；
...
| **aria2 官网 UI Frontends / Related Projects** | **事实拒收** | 列表确实存在于 `aria2/aria2.github.io` 的 `index.html:156`（Related Projects：apt-metalink / powerpill / python3-aria2jsonrpc / aria2.js）与 `index.html:165`（UI Frontends：webui-aria2 / uGet）。但该仓库 **历史 merged PR = 0**（GitHub 搜索 `repo:aria2/aria2.github.io is:merged` 返回 0 条），外部贡献进不去。且 RinaDown 不是 aria2 的前端也不是 aria2 的周边库，语义上也不属于这两个分类。 |
| **AriaNg 兼容客户端列表** | **双重不够格** | AriaNg README 指向的是 http://ariang.mayswind.net/3rd-extensions.html，源文件为 `mayswind/AriaNg-WebSite` 的 `3rd-extensions.html`。① 该页收录标准是 **「基于 AriaNg 的第三方应用」**（Server-side app with aria2 / Client-side app with aria2 / Browser add-ons，全部是打包或封装了 AriaNg 的项目），RinaDown 不包含 AriaNg；② `mayswind/AriaNg-WebSite` **历史 merged PR = 0**。 |
...
| **吾爱破解 · 原创发布区** | https://www.52pojie.cn/forum-16-1.html | 流量大但**红线也最硬**：RinaDown 是 AGPL-3.0 开源且不含破解内容，符合发布区规则；但绝对不要在帖子里提「下载付费资源」「破解 IDM」这类表述。 |
...
3. **P1 两家**：Axorax/awesome-free-apps（PR，`Add: RinaDown`）→ fmhy/edit（**issue，不是 PR**）。
...
以 `zerx-lab` 账号经 gh CLI 实际提交。外部仓库均走 fork（`$TEMP/fd-promo/<repo>`），RinaDown 主仓库未被改动。
...
**未提交**：yt-dlp（需先开 feature-request issue 讨论再写 `RinaDownFD` 代码 PR，成本高，留作后续）。
...
- 链接统一 `https://rinadown.zerx.dev`。
- 中文清单用中文正文，英文清单用英文正文；无 AI 署名、无 Co-authored-by。
- Axorax 的 commit message 严格用 `Add: RinaDown`；两条目均追加在各自分类底部，未重排、未动 TOC 与 `filter/`。
- fmhy 按其 CONTRIBUTING 走 issue（`wiki.yml` 模板，Type = Site suggestion），并在正文声明了开发者身份、请其自行测试。
- DangJin 与 fmhy 的正文均**主动声明 RinaDown 没有站点抽取器**，未把它包装成视频解析工具。

### 提交前已完成的查重

- HelloGitHub：`repo:521xueweihan/HelloGitHub RinaDown` 的 issue 与 code 搜索均 0 命中。
- ruanyf/weekly：`repo:ruanyf/weekly RinaDown` issue 搜索 0 命中。
- fmhy：`https://api.fmhy.net/single-page` 全文搜索 `RinaDown` 0 命中。

### 待办

- **§0 的 Homepage 不一致仍未修**：仓库 Homepage 字段还是 `https://www.rinadown.com`，而所有提交出去的物料写的都是 `https://rinadown.zerx.dev`。维护者核对时会看到两个域名，**应尽快统一**。
