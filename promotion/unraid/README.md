# RinaDown — Unraid Community Applications 模板

> **重要：本目录必须作为一个独立仓库 `zerx-lab/unraid-templates` 的根推送，
> 不能留在 RinaDown 主仓库里提交给 CA。**

## 为什么要独立仓库

CA 的 live scan 会**递归扫描整个仓库根**，把所有 `.xml` 都当候选 Unraid 模板解析。
RinaDown 主仓库里有 8 个无关 XML（`android/**` 的 AndroidManifest/styles/launch_background、
...
├── README.md
└── RinaDown/
    └── rinadown.xml         # 模板；TemplateURL 已指向本仓库 raw 路径
```
...
# 2. 把本目录（ca_profile.xml + README.md + RinaDown/）作为仓库根内容 push 到 main
...
- **not_unraid_application: 8**：迁出主仓库到独立干净仓库，scanner 只会看到 `RinaDown/rinadown.xml`。

## 备注

- `TemplateURL` 已指向 `raw.githubusercontent.com/zerx-lab/unraid-templates/main/RinaDown/rinadown.xml`。
  若仓库名不用 `unraid-templates`，需同步改此 URL 与 `ca_profile.xml`/`rinadown.xml` 里的 raw 链接。
- Icon 目前用 SVG。CA 界面对 SVG 支持不稳，建议改用 PNG（可从 `assets/logo/` 导出 256×256 放到本仓库
  `RinaDown/icon.png` 并改 `<Icon>` 链接）。scanner 不因此报错，仅影响显示。
