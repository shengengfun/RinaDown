...
/// 一组列 id 的总宽度。
double columnsTotalWidth(
  Iterable<TaskColumnId> ids, {
  Map<TaskColumnId, double> widths = const {},
}) => ids.fold(
  0.0,
  (sum, id) => sum + resolvedColumnWidth(id, widths),
);

/// 列实际宽度：优先用户覆盖，否则用 [kTaskColumns] 内置宽度。
double resolvedColumnWidth(
  TaskColumnId id,
  Map<TaskColumnId, double> widths,
) => widths[id] ?? kTaskColumns[id]!.width;

/// 允许用户拖拽调整宽度的列。全部 9 列皆可调；表头每条列分界线（含
/// 「文件名 ↔ 首列」分界）都是拖拽把手（见 task_list.dart / task_list_item.dart）。
const Set<TaskColumnId> kResizableColumns = {
  TaskColumnId.progress,
  TaskColumnId.size,
  TaskColumnId.created,
  TaskColumnId.protocol,
...
/// 列宽可拖拽范围（px）。
const double kColumnWidthMin = 56.0;
const double kColumnWidthMax = 800.0;

/// 表头拖拽把手 / 行占位 gap 的统一宽度（header 与行必须各放一份，
/// 否则把手宽度会把整列块顶偏，列名与行数据错位）。
const double kColResizeGap = 6.0;
...
String? tryToggleColumn({
  ...
  if (columnsTotalWidth(next, widths: widths) > columnWidthBudget(listWidth)) {
