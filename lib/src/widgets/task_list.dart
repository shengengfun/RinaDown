...
class _TaskListState extends State<TaskList> {
  ...
  /// 列宽拖拽：记录每个被拖拽列在 drag-start 时的宽度，供增量 delta 累加。
  final Map<TaskColumnId, double> _columnResizeStart = {};

  void _beginColResize(TaskColumnId id) {
    _columnResizeStart[id] = resolvedColumnWidth(id, _prefs.columnWidths);
  }

  void _colResizeDelta(TaskColumnId id, double delta) {
    final start = _columnResizeStart[id];
    if (start == null) return;
    final width = (start + delta).clamp(kColumnWidthMin, kColumnWidthMax);
    widget.viewPrefsStore.update(_tab, (p) {
      final next = Map<TaskColumnId, double>.of(p.columnWidths)..[id] = width;
      return p.copyWith(columnWidths: next);
    });
  }

  void _endColResize(TaskColumnId id) {
    _columnResizeStart.remove(id);
  }
  ...
  Widget _buildHeader(BuildContext context, ViewPrefs prefs, double listWidth) {
    ...
    final columns = isList
        ? fitColumnsToWidth(
            effectiveColumns(prefs),
            listWidth,
            widths: prefs.columnWidths,
          )
        : const <TaskColumnId>[];
    final widths = prefs.columnWidths;
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  // 每条列分界线（含「文件名 ↔ 首列」）都放一个拖拽把手，
                  // 把手调整其右侧列宽。行（TaskListItem）在同位置放等宽占位
                  // gap，二者宽度账目一致，列名才与行数据像素对齐。
                  for (var i = 0; i < columns.length; i++) ...[
                    _ColResizeHandle(
                      onResizeStart: () => _beginColResize(columns[i]),
                      onResizeUpdate: (delta) =>
                          _colResizeDelta(columns[i], delta),
                      onResizeEnd: () => _endColResize(columns[i]),
                    ),
                    SizedBox(
                      width: resolvedColumnWidth(columns[i], widths),
                      child: Padding(
                        padding: columns[i] == TaskColumnId.progress
                            ? const EdgeInsets.only(right: 12)
                            : EdgeInsets.zero,
                        child: Center(
                          child: Text(
                            kTaskColumns[columns[i]]!.label(s),
                            style: TextStyle(
                              fontSize: 11,
                              fontWeight: FontWeight.w500,
                              color: c.textMuted,
...
class _ColResizeHandle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        onHorizontalDragUpdate: (d) => onResizeUpdate(d.delta.dx),
        onHorizontalDragEnd: (_) => onResizeEnd(),
        onHorizontalDragCancel: onResizeEnd,
        child: SizedBox(width: kColResizeGap, height: 24),
      ),
    );
  }
}
