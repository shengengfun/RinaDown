...
import 'task_speed_limit_dialog.dart';
...
class _DetailPanelState extends State<DetailPanel> {
  ...
  /// 「下载分布」细节（网格 + 图例 + 分段摘要）是否展开。默认收起，只露流量图。
  ...
  Widget _buildProgress(
    ...
    final doneTotalText = task.totalBytes > 0
        ...
        : task.downloadedText;
    // 实时流量历史（无记录时图表隐藏）。
    final traffic = widget.controller.trafficOf(task.id);
      ...
      children: [
        ...
        if (traffic.isNotEmpty) ...[
          ...
          TrafficChart(
            samples: traffic,
            showUp: task.isBt,
            title: currentS.trafficTitle,
            downLabel: currentS.trafficDown,
            upLabel: currentS.trafficUp,
          ),
        ],

        // 「下载分布」细节（网格 + 图例 + 分段摘要）收进可折叠区，默认收起
        if (hasSegs) ...[
          const SizedBox(height: 14),
          _buildDistributionSection(c, m, segs!, task),
        ],
      ],
    );
  }

  /// 可折叠的「下载分布」细节区（IDM 网格 + 图例 + 分段摘要）。
  /// 交互：点标题行展开/收起；默认收起，只留流量折线图常驻。
      ...
      children: [
        MouseRegion(
          ...
          child: GestureDetector(
            ...
            onTap: () => setState(() => _distExpanded = !_distExpanded),
            child: Padding(
              padding: const EdgeInsets.symmetric(vertical: 2),
              child: Row(
                children: [
                  AnimatedRotation(
                    turns: _distExpanded ? 0.25 : 0,
        ...
        AnimatedCrossFade(
          ...
          secondChild: Column(
            ...
            children: [
              ...
              _buildSegmentGrid(c, m, segs, task.totalBytes, monochrome: mono),
              // 图例：分片过多（BT 多文件）隐藏避免溢出；BT 分片是 piece 映射
              // 而非下载线程，图例无线程归属含义，整体不显示
              if (!mono && segs.length > 1 && segs.length <= 32) ...[
                const SizedBox(height: 12),
                _buildSegmentLegend(c, m, segs),
              ],
              // segs-sum：分段数 · 活跃数 · 动态拆分状态；BT 无此语义不显示
              if (!mono) ...[
                const SizedBox(height: 12),
  ...
  Widget _buildTaskSpeedLimitRow(AppColors c, DownloadTask task) {
    ...
    final valueText = bps > 0
        ? '${valueFromBytes(unit, bps)} $unit'
    ...
    return Padding(
      ...
      child: MouseRegion(
        ...
        child: GestureDetector(
          behavior: HitTestBehavior.opaque,
          onTap: () => _showTaskSpeedLimitDialog(task),
          child: Row(
            ...
            children: [
              ...
              Expanded(
                child: Text(
                  ...
                  style: TextStyle(
                    fontSize: 11.5,
                    color: bps > 0 ? c.accent : c.textSecondary,
                  ),
                ),
              ),
  ...
  /// 单任务下载限速设置对话框（共享实现，右键菜单同款）。
  Future<void> _showTaskSpeedLimitDialog(DownloadTask task) {
    return showTaskSpeedLimitDialog(context, widget.controller, task);
  }
    ...
    return _tabScroll(
      Column(
        ...
        children: [
          ...
          _buildTaskSpeedLimitRow(c, task),
        ],
      ),
    );
  }
