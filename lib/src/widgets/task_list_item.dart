  ...
  Widget build(BuildContext context) {
    ...
    final row = Row(
      children: [
        ...
        // 每条分界线前放与表头把手等宽的占位 gap——表头把手各占 6px，
        // 行内不放等宽占位会把整列块顶偏，导致列名与行数据错位。
        for (final col in widget.columns) ...[
          const SizedBox(width: kColResizeGap),
          SizedBox(
            width: resolvedColumnWidth(col, widget.columnWidths),
...
void showTaskContextMenu(
  ...
  // --- 单任务下载限速（未完成任务）---
  if (task.status != TaskStatus.completed &&
      DownloadController.globalInstance != null) {
    items.add(
      ContextMenuItem(
        icon: LucideIcons.gauge,
        label: s.speedLimit,
        color: c.textPrimary,
        action: () => showTaskSpeedLimitDialog(
          context,
          DownloadController.globalInstance!,
          task,
        ),
      ),
    );
