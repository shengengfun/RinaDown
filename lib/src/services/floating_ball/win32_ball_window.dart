...
class Win32BallWindow {
  ...
  /// 悬停进入/离开（true = 光标在球或已展开的卡片上）。
  /// 由 FloatingBallService 注入，作为 hover 卡片的开关信号。
  ...
  // ── 悬停展开卡片（hover 卡片，仅非贴边自由球态生效）──
  bool _hovering = false; // 已上报给 Service 的悬停态
  ...
  int _cardAnchorX = 0;
  int _cardAnchorY = 0;
  double _cardOffX = 0; // 球 cell 相对窗口左上的物理偏移
  double _cardOffY = 0;

  /// 卡片态下光标离开后的防抖时长（防球/卡边界抖动闪烁）。
  static const _kCardLeaveGrace = Duration(milliseconds: 100);
  ...
  /// 是否处于贴边（含收起）态——该状态下不弹 hover 卡片，保持既有行为。
  bool get isDocked => _dockEdge != _DockEdge.none;

  /// 是否处于贴边收起态（只剩边条）。
  bool get isCollapsed => _collapsed;

  /// 是否有进行中的贴边滑出/收起动画。
  bool get isAnimating => _animStart != null;

  /// 卡片是否展开中。
  bool get isCardOpen => _cardSide != null;
  HoverCardLayout? get cardLayout => _cardLayout;
  ...
  /// 建 DIB 并贴入（记录缩放尺寸）。由 [pushImage] / 卡片几何方法共用。
  void _pushBitmap(BallImage image) {
  ...
  // ===========================================================================
  // 悬停卡片几何（动态 resize + 重定位，球始终不动）
  // ===========================================================================

  /// 计算展开卡片所需布局（纯计算，不改窗口）。供 Service 先渲染位图再提交。
  ///
  /// 仅自由（非贴边）球态调用；Service 侧已按 [isDocked] 把关。
  HoverCardLayout prepareHoverCardLayout(ui.Size cardLogicalSize) {
    final layout = layoutHoverCard(
      side: _chooseCardSide(cardLogicalSize),
  ...
  /// 按「整窗放得进当前屏工作区」挑选方向，偏好 右→左→下→上。
  BallCardSide _chooseCardSide(ui.Size cardLogicalSize) {
    final wa = _monitorWorkArea();
    if (wa != null) {
      final (waL, waT, waR, waB) = wa;
      // 球 cell 左上物理锚点（卡片收起态下窗口左上 == 球左上）
      final bx = _screenX + _cardOffX;
      final by = _screenY + _cardOffY;
      const candidates = [
        BallCardSide.right,
        BallCardSide.left,
        BallCardSide.bottom,
      ...
      for (final s in candidates) {
        final l = layoutHoverCard(
          side: s,
          cardLogicalSize: cardLogicalSize,
        );
        final offX = (l.ballOffX * scale).round();
        final offY = (l.ballOffY * scale).round();
        final winL = bx - offX;
        final winT = by - offY;
        final winR = winL + (l.totalLogicalSize.width * scale).round();
        final winB = winT + (l.totalLogicalSize.height * scale).round();
        if (winL >= waL - 2 &&
            winT >= waT - 2 &&
            winR <= waR + 2 &&
            winB <= waB + 2) {
    ...
    return BallCardSide.right;
  }

  /// 展开/刷新卡片：依 [layout] 动态 resize + 重定位并贴入 [image]。
  ///
  /// 每次调用都按球锚点重算窗口左上，因此首次展开与内容刷新（行数变化）
  /// 共用同一入口，球体屏幕位置始终不变。
  void presentHoverCard({
    ...
    final offY = (layout.ballOffY * scale).round();
    // 锚点 = 旧窗口左上 + 旧球偏移（收起态偏移为 0 → 即窗口左上）
    final anchorX = _screenX + _cardOffX.round();
    ...
    _cardAnchorY = anchorY;
    _setGeometryAndPush(image, anchorX - offX, anchorY - offY);
    logInfo(
      _tag,
      'hover card present side=${layout.side.name} '
      'win=(${anchorX - offX},${anchorY - offY} ${image.width}x${image.height})',
    );
  }

  /// 收起卡片：恢复球-only 几何（56×56）并贴入 [image]。
  void dismissHoverCard(BallImage image) {
    if (_hwnd == 0) return;
    _cardSide = null;
    _cardLayout = null;
    _cardOffX = 0;
    _cardOffY = 0;
    _hoverLeaveAt = null;
    _setGeometryAndPush(image, _cardAnchorX, _cardAnchorY);
    logInfo(_tag, 'hover card dismissed');
  }

  /// 卡片态下静默重置几何（隐藏小球等场景），把窗口左上归位到球锚点。
  /// 仅当卡片确已展开（锚点有效）时调用；不贴新图（窗口隐藏中）。
  void resetCardGeometry() {
    if (_hwnd == 0) return;
    if (_cardSide == null) return;
    _cardSide = null;
    _cardLayout = null;
    _screenX = _cardAnchorX;
    _screenY = _cardAnchorY;
    _cardOffX = 0;
    _cardOffY = 0;
    _hoverLeaveAt = null;
  }

  /// 设置窗口位置与尺寸并立即贴图（SetWindowPos + UpdateLayeredWindow 同帧）。
  void _setGeometryAndPush(BallImage image, int x, int y) {
    _screenX = x;
    _screenY = y;
    setWindowPos(
      _hwnd,
  ...
  void _onTick(Timer _) {
    ...
    final pointerHover = _pointerHover(mx, my);

    // ── 悬停态维护：向 Service 上报进入/离开 ──
    // 卡片展开态：离开经 _kCardLeaveGrace 防抖，避免球/卡边界抖动反复开合。
    if (_cardSide != null) {
      if (pointerHover) {
        _hoverLeaveAt = null;
        if (!_hovering) {
        ...
        _hoverLeaveAt ??= now.add(_kCardLeaveGrace);
        if (_hovering && now.isAfter(_hoverLeaveAt!)) {
          _hovering = false;
          _hoverLeaveAt = null;
      ...
      _hoverLeaveAt = null;
      if (pointerHover != _hovering) {
        _hovering = pointerHover;
        onHoverChanged?.call(_hovering);
      }
    }

    // 悬停/按下/动画/卡片展开中提频；空闲降频省电。
    if (pointerHover ||
        _pressedInside ||
        animating ||
        _collapseDeadline != null ||
    ...
    if (_cardSide != null && pointerHover) {
      ...
      return; // 卡片态不下发拖动/点击；用户移出或按下后由 Service 收起
    }

    // ── 右键：按下沿检测 → 上下文菜单（收起态先展开）──
    if (rightJustPressed && pointerHover && !_dragging && !_collapsed) {
      onContextMenu?.call();
      return;
    }

    // ── 左键 ──
    if (justPressed && pointerHover && !_collapsed) {
      _pressedInside = true;
      _pressX = mx;
      _pressY = my;
      _pressWinX = _screenX;
  ...
  /// 命中判定（交互用 — 悬停/按下/提频）：
  /// - 卡片展开 / 贴边收起：窗口整矩形（卡片区与窄边条都要接住悬停）
  /// - 常态：圆形（A7，圆外穿透）
  bool _pointerHover(int screenX, int screenY) {
    if (_cardSide != null || _collapsed) {
      return screenX >= _screenX &&
          screenX < _screenX + _scaledW &&
          screenY >= _screenY &&
          screenY < _screenY + _scaledH;
    }
    return _hitBall(screenX, screenY);
  }
  ...
  void _checkFullscreenAvoidance() {
    ...
    if (shouldHide && !_hiddenByFullscreen) {
      ...
      _pressedInside = false;
      _dragging = false;
      // 卡片展开中被全屏盖住 → 上报离开，让 Service 收起卡片
      if (_cardSide != null && _hovering) {
        _hovering = false;
        onHoverChanged?.call(false);
      }
