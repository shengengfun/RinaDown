      ...
      sections: [
        ...
        _SettingsGroup(
          ...
          children: [
            ...
            _SettingRow(
              label: s.appearanceBackground,
              description: s.appearanceBackgroundDesc,
              vertical: true,
              child: const _BackgroundImageSetting(),
            ),
          ],
        ),
      ],
...
/// 外观 · 自定义背景图片：选图 / 显示路径 / 移除。读 SettingsProvider 全局。
class _BackgroundImageSetting extends StatelessWidget {
  const _BackgroundImageSetting();

  static const _kImageExts = ['png', 'jpg', 'jpeg', 'webp', 'bmp', 'gif'];

  Future<void> _pick(BuildContext context) async {
    final sp = SettingsProvider.globalInstance;
    if (sp == null) return;
    final picked = await FilePickerService.pickFiles(
      dialogTitle: currentS.chooseImage,
      allowedExtensions: _kImageExts,
      allowMultiple: false,
    );
    if (picked == null || picked.isEmpty) return;
    sp.setCustomBackgroundPath(picked.first.path);
  }
  ...
  Widget build(BuildContext context) {
    final c = AppColors.of(context);
    final sp = SettingsProvider.globalInstance;
    final path = sp?.customBackgroundPath ?? '';
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Wrap(
          ...
          children: [
            ShadButton.outline(
              ...
              child: Text(currentS.chooseImage),
            ),
            if (path.isNotEmpty)
              ShadButton.destructive(
                onPressed: () => sp?.setCustomBackgroundPath(''),
                child: Text(currentS.removeBackground),
              ),
          ],
        ),
        if (path.isNotEmpty) ...[
          ...
          sections: [
            ...
            _SettingsGroup(
              ...
              children: [
                ...
                _SettingRow(
                  label: s.speedLimitUnitTitle,
                  description: s.speedLimitUnitTitleDesc,
                  child: _SpeedLimitUnitSelector(
                    settingsProvider: settingsProvider,
                  ),
                ),
                _SettingRow(
...
class _SaveDirPickerState extends State<_SaveDirPicker> {
  ...
  Widget build(BuildContext context) {
    return DirPickerField(
      ...
      onTap: _pickDir,
      onPathSelected: (dir) {
        widget.settingsProvider.setDefaultSaveDir(dir);
...
class _SpeedLimitInputState extends State<_SpeedLimitInput> {
  ...
  String get _unit => widget.settingsProvider.speedLimitUnit;

  @override
  void initState() {
    super.initState();
    _controller = TextEditingController(
      text: _valueText(widget.settingsProvider.speedLimitBytes),
    );
  }
  ...
  void didUpdateWidget(_SpeedLimitInput oldWidget) {
    ...
    final expected = _valueText(bytes);
    final current = _controller.text.trim();
    if (expected != current && (current.isEmpty || current == '0') == (bytes == 0)) {
  ...
  String _valueText(int bytes) {
    if (bytes <= 0) return '0';
    return valueFromBytes(_unit, bytes).toString();
  }

  void _onSubmit(String value) {
    final v = int.tryParse(value.trim()) ?? 0;
    widget.settingsProvider.setSpeedLimitBytes(bytesFromValue(_unit, v));
  }
...
/// 全局上传限速输入（仅作用于 BT 上传，含做种；单位跟随 [speedLimitUnit]）。
...
class _UploadLimitInputState extends State<_UploadLimitInput> {
  ...
  void initState() {
    super.initState();
    _controller = TextEditingController(
      text: _valueText(widget.settingsProvider.uploadLimitBytes),
    );
  }
  ...
  void didUpdateWidget(_UploadLimitInput oldWidget) {
    super.didUpdateWidget(oldWidget);
    final bytes = widget.settingsProvider.uploadLimitBytes;
    final expected = _valueText(bytes);
    final current = _controller.text.trim();
    if (expected != current && (current.isEmpty || current == '0') == (bytes == 0)) {
      _controller.text = expected;
  ...
  void _onSubmit(String value) {
    final v = int.tryParse(value.trim()) ?? 0;
    widget.settingsProvider.setUploadLimitBytes(bytesFromValue(_unit, v));
  }
...
/// 限速单位选择（kbps / KB/s / MB/s / mbps）。设置页「下载限速」上方。
class _SpeedLimitUnitSelector extends StatelessWidget {
  final SettingsProvider settingsProvider;

  const _SpeedLimitUnitSelector({required this.settingsProvider});

  @override
  Widget build(BuildContext context) {
    final c = AppColors.of(context);
    final current = settingsProvider.speedLimitUnit;
    return ListenableBuilder(
      listenable: settingsProvider,
      builder: (context, _) => SizedBox(
        ...
        child: ShadSelect<String>(
          ...
          options: [
            for (final u in kSpeedLimitUnits)
              ShadOption(value: u, child: Text(u)),
          ],
          selectedOptionBuilder: (context, value) => Text(
            value,
            style: TextStyle(fontSize: 12, color: c.textPrimary),
          ...
          onChanged: (v) {
            if (v != null) settingsProvider.setSpeedLimitUnit(v);
          },
        ),
      ),
    );
