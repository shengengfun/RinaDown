...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    ...
    // 性能监控（Windows：hub 每 2s 推 SystemStats）
    if (Platform.isWindows) SystemStatsService.instance.start();
  ...
  /// 自定义背景图片壁纸层（设置页/主页面共用；无图时为空）。
  /// 图片 cover 铺满 + 按明暗叠加半透明遮罩以适配对比度。
  List<Widget> _maybeBackground(AppColors c) {
    final path = _settingsProvider.customBackgroundPath;
    if (path.isEmpty) return const [];
    final dark = c.bg.computeLuminance() < 0.5;
    return [
      Positioned.fill(
        child: ColoredBox(
          color: const Color(0xFF000000), // 兜底底
          child: Stack(
            fit: StackFit.expand,
            children: [
              Image.file(
              ...
              // 自动调明暗：深色主题加深、亮色主题轻度压暗，保证前景可读。
              ColoredBox(
                color: dark ? const Color(0x55000000) : const Color(0x2B000000),
              ),
            ],
          ),
        ),
  ...
  Widget _buildContentArea(AppColors c, double totalHeight) {
    final bg = _settingsProvider.customBackgroundPath;
    // 启用壁纸时中央画布让出透出空间（侧栏/详情面板仍不透明，保可读）。
    final canvasColor = bg.isNotEmpty
