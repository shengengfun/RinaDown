...
class DownloadController extends ChangeNotifier {
  ...
  void createTask({
    ...
    if (saveDir.trim().isNotEmpty) RecentDirs.instance.push(saveDir);
    CreateTask(
      url: url,
      saveDir: saveDir,
      fileName: fileName,
  ...
  /// 本会话内各任务级下载限速（B/s；0 = 不限）。权威持久化在引擎 DB；
  /// App 重启后此表为空（回退显示「不限」直到用户再设置），UI 侧尽量
  /// 用本表 + 用户操作即时更新。
  final Map<String, int> _taskSpeedLimitBps = {};

  /// 某任务当前会话内的任务级下载限速（B/s；0 = 不限）。
  ...
  void setTaskSpeedLimit(String taskId, int bps) {
    ...
    _taskSpeedLimitBps[taskId] = v;
    _safeNotifyListeners();
    SetTaskSpeedLimit(taskId: taskId, speedLimitBps: v).sendSignalToRust();
  }
  ...
  // ---------------------------------------------------------------------------
  // 流量历史采样（迅雷式实时速率折线图数据源）
  // ---------------------------------------------------------------------------

  /// 单任务保留的最大流量样本数（~1s 一个，封顶约 1 小时）。
  static const int _kTrafficMaxSamples = 3600;

  /// 单任务流量样本（按毫秒递增）→ 会话内内存记录，不持久化。
  final Map<String, List<TrafficSample>> _traffic = {};

  /// 每个任务上次采样的时刻（1s 节流）。
  final Map<String, DateTime> _trafficLast = {};

  /// 每个任务首个采样的时刻（样本 ms 以它为 0 点）。
  final Map<String, DateTime> _trafficStart = {};

  /// 该任务的流量历史（只读快照；无记录时返回空表）。
  List<TrafficSample> trafficOf(String taskId) =>
      List.unmodifiable(_traffic[taskId] ?? const <TrafficSample>[]);

  void _dropTraffic(String taskId) {
    _traffic.remove(taskId);
    _trafficLast.remove(taskId);
    _trafficStart.remove(taskId);
  }

  /// 活跃下载期以约 1s 间隔记录一次瞬时速率（下载/上传）。
  void _recordTraffic(String taskId, int down, int up, TaskStatus status) {
    final active =
        status == TaskStatus.downloading ||
        status == TaskStatus.preparing ||
        status == TaskStatus.resuming;
    if (!active) {
      // 暂停/完成/出错：停表，但保留既有曲线供回看。
      _trafficLast.remove(taskId);
      return;
    }
    final now = DateTime.now();
    final last = _trafficLast[taskId];
    if (last != null &&
        now.difference(last) < const Duration(milliseconds: 1000)) {
      return;
    }
    final start = _trafficStart.putIfAbsent(taskId, () => now);
    _trafficLast[taskId] = now;
    final list = _traffic.putIfAbsent(taskId, () => <TrafficSample>[]);
    list.add(
      TrafficSample(
        ms: now.difference(start).inMilliseconds,
        down: down < 0 ? 0 : down,
        up: up < 0 ? 0 : up,
      ),
    );
    if (list.length > _kTrafficMaxSamples) {
      list.removeRange(0, list.length - _kTrafficMaxSamples);
    }
  }
  ...
  void _onProgress(RustSignalPack<TaskProgress> pack) {
    ...
    _recordTraffic(p.taskId, p.speed, p.uploadSpeedBps, newStatus);
    _safeNotifyListeners();
  }
...
/// 单个任务的瞬时流量样本（详情页速率折线图数据源）。
///
/// [ms] 以该任务首个样本时刻为 0 点（会话内单调递增）；[down]/[up] 单位字节/秒。
class TrafficSample {
  final int ms;
  final int down;
  final int up;
