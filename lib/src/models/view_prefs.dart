...
class ViewPrefs {
  ...
  /// 用户手动调整过的列宽（值 = 像素）。未覆盖的列用 [kTaskColumns] 内置宽度。
  final Map<TaskColumnId, double> columnWidths;
  ...
  ViewPrefs copyWith({
    ...
    Map<TaskColumnId, double>? columnWidths,
  }) => ViewPrefs(
    form: form ?? this.form,
  ...
  factory ViewPrefs.fromJson(Map<String, Object?> json) {
    ...
    return ViewPrefs(
      ...
      columnWidths: _widthsFrom(json['columnWidths'], d.columnWidths),
    );
  }

  static Map<TaskColumnId, double> _widthsFrom(
    Object? raw,
    Map<TaskColumnId, double> fallback,
  ) {
    if (raw is! Map) return fallback;
    final result = <TaskColumnId, double>{};
    for (final e in raw.entries) {
      ...
      if (id == null) continue;
      result[id] = (e.value as num).toDouble();
    }
  ...
  bool operator ==(Object other) {
    ...
    return other is ViewPrefs &&
        ...
        _mapsEqual(other.columnWidths, columnWidths);
  }

  static bool _mapsEqual(
    Map<TaskColumnId, double> a,
    Map<TaskColumnId, double> b,
  ) {
  ...
  int get hashCode => Object.hash(
    ...
    columns.fold<int>(0, (acc, c) => acc ^ c.hashCode) ^
        columnWidths.entries.fold<int>(
          0,
          (acc, e) => acc ^ e.key.hashCode ^ e.value.hashCode,
        ),
  );
}
