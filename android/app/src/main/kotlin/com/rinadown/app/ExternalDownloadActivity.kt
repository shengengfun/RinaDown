 ...
 *
 * 与 [RinadownMainActivity] 共享同一个 FlutterEngine（见 [RinadownEngine]）。share 与
...
class ExternalDownloadActivity : FlutterActivity() {
    ...
    override fun getCachedEngineId(): String? =
        if (RinadownEngine.cached != null) RinadownEngine.ENGINE_ID else null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        createdEngine = RinadownEngine.cached == null
    ...
    private fun bindAndDeliver(engine: FlutterEngine) {
        RinadownEngine.cacheIfAbsent(engine)
     ...
     * `rinadown://download` 可携带 filename/cookies/referrer/headers；X 浏览器
    ...
    private fun extractShared(intent: Intent?): HashMap<String, String>? {
        ...
        return when (intent.action) {
            ...
            Intent.ACTION_VIEW -> {
                ...
                if (data.startsWith("rinadown://", ignoreCase = true)) {
                    try {
    ...
    companion object {
        private const val SHARE_CHANNEL = "com.rinadown/share"
