 ...
 * 通过 Native Messaging 协议与 RinaDown 桌面应用通信
 *
 * 通信链路：
 *   Browser Extension
 *     <-> browser.runtime.connectNative() (stdin/stdout, 4字节LE长度前缀+JSON)
 *   rinadown_nmh.exe (中继进程)
 *     <-> Named Pipe \\.\pipe\rinadown (4字节LE长度前缀+JSON)
 *   RinaDown App
 *
 * 设计决策：
...
const NMH_NAME = "com.rinadown.nmh";
...
export interface DownloadRequest {
   ...
   * 在 form-POST 触发的下载场景下必传，否则 RinaDown 会用 GET 重发拿到错误内容。
   */
  method?: string;
...
export interface ApiResponse {
   ...
   * "local"=桌面 App（NMH），"remote"=远程 rinadown_server。
...
function getPort(): chrome.runtime.Port | null {
  ...
  try {
    ...
    console.error("[RinaDown NMH] connectNative() threw:", e);
    return null;
  }
  ...
  _port.onDisconnect.addListener((p) => {
    ...
    if (err?.message) {
      console.error("[RinaDown NMH] port disconnected, reason:", err.message);
    } else {
      console.warn("[RinaDown NMH] port disconnected (no error reason)");
    }
    _port = null;
...
async function sendWithRetry(
  ...
  if (maybeDelivered && action !== "ping") {
    ...
    if (pingResult.success) {
      console.log(
        `[RinaDown NMH] App alive after "${result.message}" — message likely delivered, skipping retry`,
      );
      return {
        success: true,
...
export async function nmhCheckRinaDownAvailable(): Promise<boolean> {
 ...
 * 带一次重连重试的可用性探测。
 *
 * 与 nmhCheckRinaDownAvailable 的区别：首次 ping 失败（超时/端口断开/app_not_running）时，
 ...
 */
export async function nmhCheckRinaDownAvailableWithRetry(): Promise<boolean> {
