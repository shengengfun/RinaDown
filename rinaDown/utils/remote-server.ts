 ...
 *     <-> fetch() HTTP/JSON（X-RinaDown-Token 鉴权）
 *   rinadown_server（headless 下载服务，与桌面 App 并列的第二条投递通道）
 *
 * 设计决策：
...
/** remote-server 所需的最小配置（对应 RinaDownSettings 的 remoteUrl/remoteToken 子集） */
export interface RemoteServerConfig {
  /** rinadown_server 地址，如 http://192.168.1.10:17800（不带尾部斜杠） */
  remoteUrl: string;
...
function buildHeaders(cfg: RemoteServerConfig): HeadersInit {
  return {
    ...
    "X-RinaDown-Client": "extension",
    "X-RinaDown-Token": cfg.remoteToken,
  };
}
 ...
 * rinadown_server 的管理 API 恒开且强制 token，/api/v1/info 是最轻量的
