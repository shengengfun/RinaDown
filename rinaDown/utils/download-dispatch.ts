...
import type { RinaDownSettings, RemoteMode } from "./settings";
...
let _settingsCache: RinaDownSettings | null = null;

async function getRoutingSettings(): Promise<RinaDownSettings> {
...
function toRoutingConfig(settings: RinaDownSettings): RoutingConfig {
  return {
...
export async function checkRinaDownAvailable(): Promise<boolean> {
  const cfg = toRoutingConfig(await getRoutingSettings());
  const mode = effectiveMode(cfg);

  if (mode === "off") {
    return nmh.nmhCheckRinaDownAvailable();
  }
  if (mode === "always") {
  ...
  const [nmhUp, remoteUp] = await Promise.all([
    nmh.nmhCheckRinaDownAvailable().catch(() => false),
 ...
 */
export async function checkRinaDownAvailableWithRetry(): Promise<boolean> {
  const cfg = toRoutingConfig(await getRoutingSettings());
  const mode = effectiveMode(cfg);

  if (mode === "off") {
    return nmh.nmhCheckRinaDownAvailableWithRetry();
  }
  if (mode === "always") {
    const result = await remotePing(cfg.remote);
    return result.success === true;
  ...
  const [nmhUp, remoteUp] = await Promise.all([
    nmh.nmhCheckRinaDownAvailableWithRetry().catch(() => false),
    remotePing(cfg.remote)
      .then((r) => r.success === true)
      .catch(() => false),
  ]);
