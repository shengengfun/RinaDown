...
const STORAGE_KEY = 'rinadown_dot_pos';
const DOT_VISIBLE_KEY = 'rinadown_dot_visible';
...
export default defineContentScript({
  ...
  async main(ctx) {
    console.log('[RinaDown UI] starting');
    ...
    browser.storage.onChanged.addListener((changes, area) => {
      ...
      if (changes['rinadown_locale']) {
        const newLocale = changes['rinadown_locale'].newValue;
        if (newLocale) {
          setLocale(newLocale);
          refreshStaticTexts();
          render();
    ...
    function buildPanel(root: HTMLElement): void {
      panelEl = h('div', 'rinadown-panel');

      const header = h('div', 'panel-header');
      header.innerHTML = `
        ${svg(SVG_LOGO, 'logo')}
        <span class="title">RinaDown</span>
    ...
    function buildFloatButton(root: HTMLElement): void {
      floatBtnEl = h('div', 'rinadown-float-btn');
    ...
    function buildQualityPicker(root: HTMLElement): void {
      qualityPickerEl = h('div', 'rinadown-quality-picker');
    ...
    function buildPreviewModal(root: HTMLElement): void {
      previewModalEl = h('div', 'rinadown-preview-modal');
      previewModalEl.innerHTML = `
        <div class="rinadown-preview-card">
