...
// 本文件新增的响应体流式部分读取为 RinaDown 自研，非 cat-catch 代码移植。
...
export default defineUnlistedScript(() => {
  // 防止重复注入
  if ((window as any).__rinadown_interceptor__) return;
  (window as any).__rinadown_interceptor__ = true;

  const RINADOWN_EVENT = "rinadown-resource-detected";
  // 拦到标准 DASH JSON manifest 时派发：权威清晰度 + 视频/音频轨 URL 列表，
  // 供 Isolated World 转发给 background 存入 tab 级 manifest store。
  const RINADOWN_DASH_EVENT = "rinadown-dash-manifest";
  ...
  function notify(
    ...
    document.dispatchEvent(
      new CustomEvent(RINADOWN_EVENT, {
  ...
  function tryDashManifest(obj: unknown, url: string): void {
    try {
      ...
      if (manifest) {
        document.dispatchEvent(
          new CustomEvent(RINADOWN_DASH_EVENT, {
  ...
  XMLHttpRequest.prototype.open = function (
    ...
    try {
      ...
      (this as any).__rinadown_url = urlStr;
  ...
  XMLHttpRequest.prototype.send = function (...args: any[]) {
    this.addEventListener("load", function () {
      try {
        const url = (this as any).__rinadown_url as string | undefined;
        if (!url) return;
  ...
  // 通过拦截 AJAX 响应，在浏览器发起 CDN GET 之前将 URL 发给 RinaDown，
  // 并由 background 通过 declarativeNetRequest 阻断浏览器的 CDN 请求，
  // 确保 RinaDown 是第一个（也是唯一的）请求方。
  //
  // 目前支持的规则：
  ...
  function notifyPreemptDownload(json: any, apiUrl: string): void {
    ...
    if (
      ...
      document.dispatchEvent(
        new CustomEvent("rinadown-preempt-download", {
          detail: {
            url: cdnUrl,
  ...
  console.log("[RinaDown] Fetch/XHR/Blob/MSE interceptor injected");
});
