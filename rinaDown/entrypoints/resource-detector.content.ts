  ...
  async main(ctx) {
    ...
    if (sniffingEnabled) {
      try {
        ...
        console.warn("[RinaDown] Failed to inject fetch interceptor:", e);
      }
    }
    ...
    document.addEventListener("rinadown-resource-detected", handleFetchEvent);
    ctx.onInvalidated(() => {
      document.removeEventListener(
        "rinadown-resource-detected",
        handleFetchEvent,
      );
    });
    ...
    document.addEventListener("rinadown-dash-manifest", handleDashManifestEvent);
    ctx.onInvalidated(() => {
      document.removeEventListener(
        "rinadown-dash-manifest",
    ...
    // 立刻转发给 background，在浏览器发起 CDN GET 之前通知 RinaDown。
    ...
    document.addEventListener("rinadown-preempt-download", handlePreemptEvent);
    ctx.onInvalidated(() =>
      document.removeEventListener(
        "rinadown-preempt-download",
        handlePreemptEvent,
      ),
    );
    ...
    // 改由 RinaDown 接管。使用捕获阶段，早于页面自身的 click 处理器执行。
