/**
 * RinaDown Background Service Worker
 *
 * 职责：
 * 1. 拦截浏览器下载事件，转发给 RinaDown 桌面应用
 * 2. 注册右键菜单（发送链接到 RinaDown）
...
import {
  ...
  checkRinaDownAvailable,
  checkRinaDownAvailableWithRetry,
  warmupNativeHost,
...
export default defineBackground(() => {
  console.log("[RinaDown] Background service worker started");

  // ===== P3: settings 内存缓存 =====
  // 避免每次拦截都 await chrome.storage.sync.get（热路径去异步化）
  // storage.onChanged 保证跨标签页/窗口的实时同步
  let _settingsCache: import("@/utils/settings").RinaDownSettings | null = null;
  ...
  let _settingsInflight: Promise<
    import("@/utils/settings").RinaDownSettings
  > | null = null;
  ...
  getCachedSettings()
    .then((s) => {
      ...
      syncDownloadShelfState(s.enabled);
      console.log("[RinaDown] Settings cache warmed up");
    })
    .catch((e) => {
      console.warn("[RinaDown] Settings cache warmup failed (non-fatal):", e);
    });
  ...
  initI18n()
    .then(() => {
      console.log("[RinaDown] i18n initialized");
      // i18n 就绪后创建右键菜单（菜单标题需要翻译）
      createContextMenus();
    })
    .catch((e) => {
      console.warn("[RinaDown] i18n init failed (non-fatal):", e);
    });
  ...
  // ===== 右键菜单：即使关闭自动拦截也可以手动发送链接到 RinaDown 下载 =====
  ...
  function createContextMenus() {
    if (!browser.contextMenus) {
      console.warn("[RinaDown] contextMenus API not available");
      return;
    }
    browser.contextMenus
      ...
      .then(() => {
        browser.contextMenus.create({
          id: "rinadown-send-link",
          title: t("contextMenu.sendToRinaDown"),
          contexts: ["link"],
        });
        browser.contextMenus.create({
          id: "rinadown-send-image",
          title: t("contextMenu.sendImageToRinaDown"),
          contexts: ["image"],
        });
        browser.contextMenus.create({
          id: "rinadown-send-video",
          title: t("contextMenu.sendVideoToRinaDown"),
          contexts: ["video", "audio"],
        });
        browser.contextMenus.create({
          id: "rinadown-send-page",
          title: t("contextMenu.sendPageToRinaDown"),
          contexts: ["page"],
        });
        console.log("[RinaDown] Context menus created");
      })
      .catch((e: unknown) => {
        console.warn("[RinaDown] Failed to create context menus:", e);
      });
  }
    ...
    browser.contextMenus.onClicked.addListener(
      async (info: chrome.contextMenus.OnClickData, tab?: chrome.tabs.Tab) => {
        ...
        switch (info.menuItemId) {
          case "rinadown-send-link":
            downloadUrl = info.linkUrl;
            break;
          case "rinadown-send-image":
          case "rinadown-send-video":
            downloadUrl = info.srcUrl;
            break;
          case "rinadown-send-page":
            downloadUrl = info.pageUrl;
            break;
          default:
            return; // 非 RinaDown 菜单项，忽略
        }
        ...
        console.log(
          "[RinaDown] Context menu download:",
          info.menuItemId,
          downloadUrl,
        );

        const sendOk = await sendToRinaDown(downloadUrl, referrer);
        // 成功提示已收敛到 sendToRinaDown 内部统一发送，此处只处理失败回退。
        if (!sendOk) {
  ...
  browser.commands.onCommand.addListener(async (command) => {
    ...
    console.log("[RinaDown] Intercept toggled via shortcut:", newEnabled);
  });
  ...
  // 触发下载的站点，RinaDown 拿到 URL 后用 GET 重发会拿到 HTML 页面而非
  ...
  // method/body 一起塞进发送给 RinaDown App 的 payload。Rust 端用此
  // 一比一重建浏览器请求事务。
  //
  ...
  try {
    ...
    console.log(
      "[RinaDown] webRequest.onBeforeRequest listener registered (with requestBody)",
    );
  } catch (e) {
    console.warn(
      "[RinaDown] Failed to register onBeforeRequest listener:",
      e,
    );
  }
  ...
  try {
    ...
    console.log(
      "[RinaDown] webRequest.onSendHeaders listener registered (with extraHeaders)",
    );
  } catch {
    try {
      browser.webRequest.onSendHeaders.addListener(
      ...
      console.log(
        "[RinaDown] webRequest.onSendHeaders listener registered (without extraHeaders)",
      );
    } catch (e) {
      console.warn(
        "[RinaDown] Failed to register webRequest.onSendHeaders listener:",
        e,
      );
    }
  }
   ...
   * 其余（Accept、Sec-Fetch-*、Accept-Language 等）全部原样传递给 RinaDown。
   *
  ...
  try {
    browser.webRequest.onHeadersReceived.addListener(
      (details) => {
        ...
        console.log(
          "[RinaDown] Detected download-triggering response (onHeadersReceived):",
          info,
        );
    ...
    console.log("[RinaDown] webRequest.onHeadersReceived listener registered");
  } catch (e) {
    console.warn(
      "[RinaDown] Failed to register webRequest.onHeadersReceived listener:",
      e,
    );
  }
  ...
  try {
    browser.webRequest.onHeadersReceived.addListener(
      (details) => {
        ...
        // 仍能携带正确的认证头发送给 RinaDown——这是 IDM 能成功而普通插件失败的关键。
    ...
    console.log(
      "[RinaDown] Resource sniffer (onHeadersReceived for media) registered",
    );
  } catch (e) {
    console.warn("[RinaDown] Failed to register resource sniffer:", e);
  }
  ...
  // onDeterminingFilename 检查此表，避免重复发送给 RinaDown。
  ...
  const APP_DOWN_STATE_KEY = "rinadownAppDownUntil";
  let _appDownUntil = 0;
  ...
  function probeAppReachable(): Promise<boolean> {
    if (!_appProbe) {
      _appProbe = checkRinaDownAvailableWithRetry()
        .catch(() => false)
        .finally(() => {
          _appProbe = null;
        });
  ...
  // 同步做拦截决策，异步 fire-and-forget 发送到 RinaDown（不阻塞响应管线）。
  //
  ...
  if (!hasDeterminingFilename) {
    const onDownloadHeadersBlocking = (
      ...
      // 命中：同步取消该请求（浏览器不创建下载项），异步发送到 RinaDown
      console.log("[RinaDown] Firefox blocking intercept (cancel at source):", {
        url: details.url,
        contentType,
        contentLength,
        dispositionFilename,
      ...
      sendToRinaDown(
        ...
        .then((ok) => {
          if (!ok) {
            console.warn(
              "[RinaDown] Firefox blocking: send failed, falling back to browser download:",
              details.url,
            );
        ...
        .catch((e) => {
          console.error(
            "[RinaDown] Firefox blocking: sendToRinaDown threw:",
            e,
          );
    ...
    try {
      ...
      console.log(
        "[RinaDown] Firefox blocking download interceptor registered",
      );
    } catch (e) {
      console.warn(
        "[RinaDown] Failed to register Firefox blocking interceptor:",
        e,
      );
    }
  }
  ...
  browser.downloads.onCreated.addListener((downloadItem) => {
    ...
    console.log("[RinaDown] onCreated:", {
      id: downloadId,
      url,
    ...
    startFallbackInterception(downloadId, downloadItem).catch((e) => {
      console.error(
        "[RinaDown] Unexpected error in startFallbackInterception:",
        e,
      );
    });
  ...
  async function startFallbackInterception(
    ...
    console.log("[RinaDown] startFallbackInterception:", {
      id: downloadId,
      url,
  ...
  async function handleResponseCacheHit(
    ...
    if (handledDownloads.has(downloadId)) return;
    console.log("[RinaDown] Fallback path A (response cache):", {
      id: downloadId,
      url,
    ...
    console.log("[RinaDown] Path A shouldIntercept:", intercept, itemInfo);
    if (!intercept) return;
    if (handledDownloads.has(downloadId)) return;
    ...
    try {
      await executeFallbackIntercept(
        ...
        itemInfo,
        // 重定向场景：传入原始 URL，让 sendToRinaDown 可回退查找 headers 缓存
  ...
  async function handleDownloadMetaHit(
    ...
    if (handledDownloads.has(downloadId)) return;
    console.log("[RinaDown] Fallback path B (download meta):", {
      id: downloadId,
    ...
    if (
      ...
      console.log(
        "[RinaDown] Path B: download already complete/interrupted, skip",
      );
      return;
    }
    ...
    const intercept = shouldIntercept(itemInfo, settings);
    console.log("[RinaDown] Path B shouldIntercept:", intercept, itemInfo);
    if (!intercept) return;
    if (handledDownloads.has(downloadId)) return;
  ...
  /**
   * 执行兜底拦截：cancel + erase + 发送到 RinaDown
   *
   * 策略：先取消浏览器下载，再发送到 RinaDown。
   * 原因：如果先发送再取消，异步等待期间浏览器下载持续进行，
   * 小文件可能在 cancel 前已完成，导致"双下载"（用户同时看到
   * RinaDown 下载和浏览器已完成的下载）。Firefox 尤为严重，
  ...
  async function executeFallbackIntercept(
    ...
    try {
      ...
      console.error(
        "[RinaDown] executeFallbackIntercept: sendToRinaDown threw:",
        e,
      );
    }
    ...
    if (!sendOk) {
      ...
      console.warn(
        "[RinaDown] executeFallbackIntercept: send failed, checking app status before fallback:",
        url,
      );
      handledDownloads.delete(downloadId);
    ...
    browser.downloads.onDeterminingFilename.addListener(
      (downloadItem, suggest) => {
        ...
        if (handledDownloads.get(downloadItem.id) === "fallback") {
          console.log(
            "[RinaDown] onDeterminingFilename: already handled by fallback, cancelling:",
            downloadItem.id,
          );
        ...
        if (preemptEntry && preemptEntry.expiry > Date.now()) {
          if (downloadUrl === url) {
            ...
            console.log(
              "[RinaDown] onDeterminingFilename: preempted URL, no redirect detected, letting browser handle:",
              url,
            );
          ...
          console.log(
            "[RinaDown] onDeterminingFilename: preempted URL redirected, intercepting finalUrl:",
            downloadUrl,
          );
        }
        ...
        if (_syncSettings !== null) {
          ...
          if (shouldIntercept(_syncItemInfo, _syncSettings)) {
            ...
            suggest();
            console.log("[RinaDown] Intercepting download (sync-path):", {
              url,
              downloadUrl,
              mime: _syncMime,
              filename: _syncFilename,
            ...
            (async () => {
              try {
                try {
                  ...
                  console.debug(
                    "[RinaDown] sync-path: cancel after suggest (expected)",
                  );
                }
                try {
                  ...
                  console.debug(
                    "[RinaDown] sync-path: erase after cancel (expected)",
                  );
                }
                ...
                const sendOk = await sendToRinaDown(
                  downloadUrl,
                  _syncReferrer,
                  _syncClean,
                  _syncFileSize,
                ...
                console.error("[RinaDown] sync-path: sendToRinaDown error:", e);
        ...
        if (_syncSettings === null) {
          ...
          suggest();
          console.log(
            "[RinaDown] Cold-start pre-emptive intercept (settings cache not warmed):",
            { url, downloadUrl },
          );
          (async () => {
            try {
              ...
              try {
                ...
                console.debug("[RinaDown] cold-start: cancel (expected)");
              }
              try {
                await browser.downloads.erase({ id: downloadItem.id });
              } catch {
                console.debug("[RinaDown] cold-start: erase (expected)");
              }
              ...
              console.error(
                "[RinaDown] Cold-start pre-emptive intercept error:",
                e,
              );
              handledDownloads.delete(downloadItem.id);
              // 异常情况：先 ping 确认 App 是否在线再决定是否回退
        ...
        (async () => {
          ...
          // 防止 sendToRinaDown 失败时 catch 块误删 handledDownloads 标记导致重复发送。
          ...
          const callSuggestCancel = async () => {
            ...
            try {
              ...
              console.debug(
                "[RinaDown] async-path: cancel after suggest (expected)",
              );
            }
            try {
              await browser.downloads.erase({ id: downloadItem.id });
              ...
              console.debug(
                "[RinaDown] async-path: erase after cancel (expected)",
              );
            }
          };
          ...
          try {
            ...
            console.log(
              "[RinaDown] Intercepting download (onDeterminingFilename):",
              {
                url,
                downloadUrl,
                mime,
            ...
            console.error(
              "[RinaDown] Error in onDeterminingFilename handler:",
              e,
            );
    ...
    _queueChain = _queueChain.then(async () => {
      try {
        ...
        if (!isDup) {
          ...
          console.log("[RinaDown] Enqueued pending request:", request.url);
        }
      } catch (e) {
        console.warn("[RinaDown] Failed to enqueue pending request:", e);
      }
    });
    return _queueChain;
  }
    ...
    _queueChain = _queueChain.then(async () => {
      ...
      for (const item of queue) {
        if (item.retries >= MAX_PENDING_RETRIES) {
          console.warn(
            "[RinaDown] Pending request exceeded max retries, dropping:",
            item.request.url,
          );
          continue; // 丢弃
        }
      ...
      for (let i = 0; i < toRetry.length; i++) {
        ...
        if (result.status === "fulfilled" && result.value.success) {
          console.log("[RinaDown] Pending request flushed:", item.request.url);
  ...
  // 活跃感知来源：
  //   a) 本地下载请求发送成功后（sendToRinaDown / batchDownload），乐观地
  //      认为"可能有活跃任务"，启动 alarm；
  ...
  const TASK_POLL_ALARM_NAME = "rinadown-task-poll";
  ...
  async function ensureTaskPollAlarm(): Promise<void> {
    try {
      ...
      console.warn("[RinaDown] ensureTaskPollAlarm failed:", e);
    }
  }
  ...
  // storage.local。
  const TASK_SNAPSHOT_SESSION_KEY = "rinadownTaskSnapshot";
  const TASK_SNAPSHOT_LOCAL_KEY = "rinadownTaskSnapshotFallback";
  ...
  const TASK_NOTIFICATION_PREFIX = "rinadown-task-";
  ...
  function notifyTaskCompleted(task: TaskBrief): void {
    ...
    try {
      ...
      console.warn("[RinaDown] notifyTaskCompleted: failed to create:", e);
    }
  }
  ...
  browser.alarms.onAlarm.addListener((alarm) => {
    ...
    nmhListTasks()
      ...
      .catch((e) => {
        console.warn("[RinaDown] task poll alarm handler failed:", e);
      });
  });
  ...
  // rinadown:// 协议（Windows 注册表 / Linux x-scheme-handler / macOS
  ...
  /**
   * 打开 rinadown://download?url=...&filename=... 协议 URL，触发系统唤起
  ...
  async function openProtocolUrl(
    ...
    const protocolUrl = `rinadown://download?${params.toString()}`;
    console.log("[RinaDown] protocol mode — opening:", protocolUrl);
    try {
      ...
      console.warn("[RinaDown] failed to open rinadown:// URL:", e);
      return false;
    }
  }

  // ===== 核心：发送下载请求到 RinaDown App =====
  async function sendToRinaDown(
    url: string,
    referrer?: string,
    filename?: string,
    fileSize?: number,
    ...
    // === rinadown:// 自定义协议模式 ===
    // 开启后跳过 NMH/远端投递，经协议 URL 唤起 RinaDown 客户端
    ...
    // 限制：不经过 NMH，Cookie/Headers/method/body 无法携带，适用于公开文件。
    {
      ...
      if (protocolSettings.enableRinadownProtocol) {
        if (audioUrl) {
          // 音视频分轨对（video+audio mux）无法经协议 URL 表达 —— 丢弃
          // audioUrl 会下成无声视频。返回失败让调用方回退浏览器下载。
          console.warn(
            "[RinaDown] protocol mode cannot carry audioUrl; falling back to browser download",
          );
          ...
          if (ok && (await shouldNotifyChannel("local"))) {
            ...
            notify(
              ...
              t("notify.sentToRinaDown", { name: shownName }),
            );
          }
          return ok;
        }
    ...
    if (authFromCache.cookies || authFromCache.headers) {
      console.log(
        "[RinaDown] Cookies from webRequest cache:",
        cookieString.length,
        "chars",
      );
      console.log(
        "[RinaDown] Extra headers from webRequest cache:",
    ...
    if (!cookieString) {
      try {
        ...
        cookieString = cookiesResult
          ...
          .join("; ");
        console.log(
          "[RinaDown] Cookies from cookies API:",
          cookiesResult.length,
          "cookies",
        );
      } catch (e) {
        console.warn(
          "[RinaDown] Failed/timeout to extract cookies via API:",
          e,
        );
      }
    }
    ...
    if (!cookieString && storedCookies) {
      ...
      console.log(
        "[RinaDown] Cookies from stored resource:",
        cookieString.length,
        "chars",
      );
    }
    if (
      ...
      console.log(
        "[RinaDown] Extra headers from stored resource:",
        Object.keys(extraHeaders).length,
      );
    }
    ...
    if (reqRecord.method || reqRecord.body) {
      console.log(
        "[RinaDown] Captured original request transaction: method=",
    ...
    console.log("[RinaDown] Sending to RinaDown app:", request);
    ...
    if (response.success) {
      ...
      await incrementStat("failed");
      console.warn(
        "[RinaDown] sendToRinaDown failed:",
        response.message,
        "url:",
        url,
      );
  ...
  /**
   * 回退到浏览器下载：当发送到 RinaDown 失败时，重新发起浏览器下载。
  ...
  async function fallbackToBrowserDownload(
    ...
    try {
      ...
      console.log("[RinaDown] Fallback: re-initiated browser download:", url);
    } catch (e) {
      console.error(
        "[RinaDown] Fallback: failed to re-initiate browser download:",
        e,
      );
    }
    if (!silent) {
  ...
  /**
   * sendToRinaDown 失败后的智能回退：先 ping 确认 App 状态再决定是否回退。
   *
  ...
  async function fallbackAfterSendFailure(
    ...
    try {
      ...
      if (appAlive) {
        ...
        markAppUp();
        console.log(
          "[RinaDown] App is alive after send failure — skipping browser fallback (message likely delivered):",
          url,
        );
        return;
      }
  ...
  async function handleMessage(
    ...
    switch (message.action) {
      // --- Popup 消息 ---
      case "getStatus": {
        const available = await checkRinaDownAvailable();
      ...
      case "checkConnection": {
        const isAvailable = await checkRinaDownAvailable();
      ...
      // 远程通道（rinadown_server）：单次 POST /download/batch 发送全部条目。
      case "batchDownload": {
        ...
        // Bug 9 修复：cookies API 加 500ms 超时，与 sendToRinaDown 保持一致
      ...
      // 才能获取真正的下载 URL。因此不再发送给 RinaDown，仅记录 URL 做去重。
      ...
      case "preemptDownload": {
        ...
        console.log(
          "[RinaDown] preemptDownload: recorded URL (not sent to RinaDown, browser will load transit page):",
          preemptUrl,
        );
  ...
  async function cancelAndErase(downloadId: number): Promise<void> {
    try {
      ...
      console.warn("[RinaDown] cancelAndErase: cancel failed:", e);
    }
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      try {
        ...
        console.warn("[RinaDown] cancelAndErase: erase failed:", e);
      }
      try {
    ...
    console.warn(
      "[RinaDown] cancelAndErase: record still present after retries:",
      downloadId,
    );
  }
  ...
  function notify(title: string, message: string) {
    ...
    try {
      browser.notifications.create({
        type: "basic",
        iconUrl: "/icon/128.png",
        title: `RinaDown - ${title}`,
        message,
      });
    } catch (e) {
      console.warn("[RinaDown] notify: failed to create notification:", e);
    }
  }
