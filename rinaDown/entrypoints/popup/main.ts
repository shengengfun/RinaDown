...
import { checkRinaDownAvailable } from '@/utils/download-dispatch';
...
async function refreshConnectionStatus(): Promise<void> {
  ...
  const available = await checkRinaDownAvailable().catch(() => false);
...
async function init() {
  const [, localState, settings] = await Promise.all([
    initI18n(),
    browser.storage.local.get(['theme', 'rinadown_dot_visible', 'stats']),
    loadSettings(),
  ]);
  ...
  protocolToggle.checked = settings.enableRinadownProtocol === true;
  updateProtocolHint(settings.enableRinadownProtocol === true);
  dotVisibleToggle.checked = localState?.['rinadown_dot_visible'] !== false;
...
dotVisibleToggle.addEventListener('change', async () => {
  await browser.storage.local.set({ rinadown_dot_visible: dotVisibleToggle.checked });
});
...
protocolToggle.addEventListener('change', async () => {
  await saveSettings({ enableRinadownProtocol: protocolToggle.checked });
...
init().catch((e) => {
  console.error('[RinaDown Popup] Init failed:', e);
});
