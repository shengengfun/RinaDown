//! RinaDown auto-updater helper binary.
//!
//! Spawned by the main RinaDown process right before it exits.  Waits for the
...
//!   rinadown_updater --pid PID --zip PATH --dir PATH --exe NAME
//!
//! ### Windows – NSIS installer
//!   rinadown_updater --pid PID --installer PATH
//!
//! ### Linux – AppImage replacement
//!   rinadown_updater --pid PID --appimage-src NEW --appimage-dst CURRENT
//!
//! ### Linux – portable tar.gz
//!   rinadown_updater --pid PID --tarball PATH --dir PATH --exe NAME
//!
//! ### Linux – deb package (pkexec)
//!   rinadown_updater --pid PID --package-deb PATH
//!
//! ### Linux – Arch package (pkexec)
//!   rinadown_updater --pid PID --package-arch PATH
//!
//! ### macOS – .app bundle replacement
//!   rinadown_updater --pid PID --tarball PATH --app-bundle NAME --install-dir PATH
...
fn main() {
    if let Err(e) = run() {
        let msg = format!("rinadown_updater error: {e}");
        let _ = log_msg(&msg);
        eprintln!("{msg}");
        process::exit(1);
    }
...
fn do_portable_zip(zip: &Path, dir: &Path, exe: &str) -> Result<(), UpdaterError> {
    ...
    // rinadown_updater.exe even though we are currently running.
    // A running .exe on Windows cannot be deleted but CAN be renamed.
    #[cfg(target_os = "windows")]
    let old_self = rename_self_aside();

    // The NMH relay (rinadown_nmh.exe) lives in the same directory but is
    ...
    let tmp = std::env::temp_dir().join(format!("rinadown_upd_{}", process::id()));
    extract_zip(zip, &tmp)?;
        ...
        write_failure_marker(
            ...
            &format!(
                "Automatic update could not replace the program files \
                 ...
                 Please copy its contents over your RinaDown folder manually, \
                 or download the latest version from https://rinadown.zerx.dev",
                src.display()
            ),
        );
...
/// ZIP extraction can overwrite `rinadown_updater.exe`.  On Windows a running
...
///
/// `rinadown_nmh.exe` is the Native Messaging relay: it is spawned and owned by
/// the **browser**, not by the RinaDown app, so it can still be running after
...
fn terminate_residual_processes(install_dir: &Path) {
    const SIDECARS: &[&str] = &["rinadown_nmh.exe"];
...
/// kill an unrelated RinaDown install elsewhere on the machine.
#[cfg(target_os = "windows")]
...
/// such as `RinaDown-v1.2.3-windows/`), return that child as the real source
...
fn log_msg(msg: &str) -> io::Result<()> {
    ...
    let path = std::env::temp_dir().join("rinadown_updater.log");
