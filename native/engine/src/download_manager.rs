...
fn is_retriable_error(msg: &str) -> bool {
    ...
    lower.contains("stalled")
        ...
        || lower.contains("segment 0 failed")
        || lower.contains("segment ") && lower.contains(" failed after ")
...
pub struct ResolvePreviewOutcome {
    ...
    /// 无错误时为空。
    pub error: String,
    pub variants: Vec<crate::model::ResolveVariantOption>,
    pub file_name: String,
    pub total_bytes: i64,
...
impl DownloadManager {
    ...
    pub async fn begin_resolve_preview(
        ...
        tokio::spawn(async move {
            ...
            sink.emit(EngineEvent::ResolvePreviewReady {
                ...
                resolver_identity: outcome.resolver_identity,
    ...
    pub fn spawn_resolve_preview(
        ...
        handle.spawn(async move {
            let identity = if video {
                pm.match_resolver(&url).await
            } else {
                pm.match_multi_resolver(&url).await
            };
            ...
            let outcome = match result {
                Ok(Some(res)) if video => ResolvePreviewOutcome {
                    ...
                    range_supported: res.range_supported,
    ...
    async fn do_start_task(&mut self, queued: QueuedTask) {
        ...
        // Select speed limiter: per-task (if set) > queue-specific > global.
        let speed_limiter = self.effective_limiter_for(&task_id, &queue_id).await;
    ...
    async fn do_resume_task(&mut self, task_id: &str) {
        ...
        // (per-task if set > queue-specific > global).
        let speed_limiter = self.effective_limiter_for(task_id, &task.queue_id).await;
