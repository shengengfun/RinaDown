...
fn task_from_row(row: &AnyRow) -> Result<TaskInfo, sqlx::Error> {
    Ok(TaskInfo {
        ...
        speed_limit_bps: row.try_get("speed_limit_bps").unwrap_or(0),
...
impl Db {
    ...
    async fn init_schema(&self) -> Result<(), DbError> {
        ...
        self.add_column_if_missing("queues", "upload_limit_kbps", "BIGINT NOT NULL DEFAULT 0")
            .await?;
        // 任务级下载限速（B/s；0 = 无单任务限制，跟随队列/全局）。
        // 任务启动/续传时优先于队列级与全局限速，见 download_manager。
        self.add_column_if_missing("tasks", "speed_limit_bps", "BIGINT NOT NULL DEFAULT 0")
            .await?;
        Ok(())
    }
    ...
    /// 写入任务级下载限速（B/s，0 = 不限）。任务详情「限速」面板持久化。
    pub async fn set_task_speed_limit(&self, task_id: &str, bps: i64) -> Result<(), DbError> {
        sqlx::query("UPDATE tasks SET speed_limit_bps = $1 WHERE id = $2")
            .bind(bps.max(0))
            .bind(task_id)
            .execute(&self.pool)
            .await?;
