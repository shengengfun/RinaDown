...
pub async fn run(db_dir: PathBuf) -> Result<(), ActorError> {
    ...
    let group_tx = aux_tx.clone();
    {
        ...
        let task_speed_recv = SetTaskSpeedLimit::get_dart_signal_receiver();
        ...
        tokio::spawn(async move {
            loop {
                tokio::select! {
                    ...
                    Some(signal) = task_speed_recv.recv() => {
                        if group_tx.send(AuxSignal::TaskSpeedLimit(signal.message)).is_err() { break; }
                    }
    ...
    loop {
        tokio::select! {
            Some(signal) = create_recv.recv() => {
                ...
                engine.manager
                    .create_task(NewTaskSpec {
                        ...
                        resolver_item: msg.resolver_item,
                        ..Default::default()
                    })
                    .await;
            ...
            Some(aux) = aux_rx.recv() => {
                ...
                AuxSignal::TaskSpeedLimit(msg) => {
                    engine
                        .manager
                        .set_task_speed_limit(&msg.task_id, msg.speed_limit_bps as u64)
                        .await;
                }
