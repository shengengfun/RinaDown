...
pub struct CreateTask {
    ...
    /// 已在视频解析窗口选定的变体索引，格式为 `__flux_variant:<index>`。
    #[serde(default)]
...
/// 系统性能采样（Rust → Dart，Windows，约 2s 一次）。供状态栏
/// 「限速」左侧的 CPU/内存/网络芯片展示。
#[derive(Serialize, RustSignal)]
...
pub struct ResolvePreviewResult {
    ...
    /// 视频解析入口返回的画质/格式选项。
    #[serde(default)]
    pub variants: Vec<ResolveVariantOption>,
    #[serde(default)]
    pub file_name: String,
    #[serde(default)]
    pub total_bytes: i64,
